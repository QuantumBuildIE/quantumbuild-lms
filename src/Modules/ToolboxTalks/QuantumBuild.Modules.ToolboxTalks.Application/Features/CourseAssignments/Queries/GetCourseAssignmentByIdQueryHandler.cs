...
public class GetCourseAssignmentByIdQueryHandler : IRequestHandler<GetCourseAssignmentByIdQuery, ToolboxTalkCourseAssignmentDto?>
{
    private readonly IToolboxTalksDbContext _dbContext;

    public GetCourseAssignmentByIdQueryHandler(IToolboxTalksDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public async Task<ToolboxTalkCourseAssignmentDto?> Handle(GetCourseAssignmentByIdQuery request, CancellationToken cancellationToken)
    {
        var assignment = await _dbContext.ToolboxTalkCourseAssignments
            .Where(a => a.Id == request.Id && a.TenantId == request.TenantId && !a.IsDeleted)
            .Include(a => a.Course)

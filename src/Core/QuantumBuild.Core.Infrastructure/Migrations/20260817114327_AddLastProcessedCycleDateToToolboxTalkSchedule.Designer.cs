// <auto-generated />
using System;
using System.Collections.Generic;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using QuantumBuild.Core.Infrastructure.Data;

#nullable disable

namespace QuantumBuild.Core.Infrastructure.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20260817114327_AddLastProcessedCycleDateToToolboxTalkSchedule")]
    partial class AddLastProcessedCycleDateToToolboxTalkSchedule
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.18")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<System.Guid>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("text");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("text");

                    b.Property<Guid>("RoleId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("RoleClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<System.Guid>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("text");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("text");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("UserClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<System.Guid>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("text");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("text");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("text");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("UserLogins", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<System.Guid>", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .HasColumnType("text");

                    b.Property<string>("Value")
                        .HasColumnType("text");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("UserTokens", (string)null);
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.BulkImportSession", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("CsvR2Key")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<string>("DeletedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsRerun")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("ProcessingResultJson")
                        .HasColumnType("text");

                    b.Property<DateTimeOffset?>("ProcessingStartedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasDefaultValue("Uploaded");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<DateTimeOffset>("UploadedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("ValidationResultJson")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("UploadedAt")
                        .HasDatabaseName("IX_BulkImportSessions_UploadedAt");

                    b.HasIndex("TenantId", "Status")
                        .HasDatabaseName("IX_BulkImportSessions_TenantId_Status");

                    b.ToTable("BulkImportSessions");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.BulkSopImportSession", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsRerun")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("ProcessingResultJson")
                        .HasColumnType("text");

                    b.Property<DateTimeOffset?>("ProcessingStartedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasDefaultValue("Uploaded");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<DateTimeOffset>("UploadedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("ValidationResultJson")
                        .HasColumnType("text");

                    b.Property<string>("ZipR2Key")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.HasKey("Id");

                    b.HasIndex("UploadedAt")
                        .HasDatabaseName("IX_BulkSopImportSessions_UploadedAt");

                    b.HasIndex("TenantId", "Status")
                        .HasDatabaseName("IX_BulkSopImportSessions_TenantId_Status");

                    b.ToTable("BulkSopImportSessions");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.Company", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("AddressLine1")
                        .HasColumnType("text");

                    b.Property<string>("AddressLine2")
                        .HasColumnType("text");

                    b.Property<string>("City")
                        .HasColumnType("text");

                    b.Property<string>("CompanyCode")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("CompanyName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("CompanyType")
                        .HasColumnType("text");

                    b.Property<string>("Country")
                        .HasColumnType("text");

                    b.Property<string>("County")
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("Email")
                        .HasColumnType("text");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<string>("Notes")
                        .HasColumnType("text");

                    b.Property<string>("Phone")
                        .HasColumnType("text");

                    b.Property<string>("PostalCode")
                        .HasColumnType("text");

                    b.Property<string>("RegistrationNumber")
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<string>("TradingName")
                        .HasColumnType("text");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("text");

                    b.Property<string>("VatNumber")
                        .HasColumnType("text");

                    b.Property<string>("Website")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Companies");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.Contact", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CompanyId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("Email")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsPrimaryContact")
                        .HasColumnType("boolean");

                    b.Property<string>("JobTitle")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("Mobile")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("Notes")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<string>("Phone")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<Guid?>("SiteId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("CompanyId");

                    b.HasIndex("SiteId");

                    b.ToTable("Contacts");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.Department", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Code")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("TenantId", "Name")
                        .IsUnique()
                        .HasDatabaseName("IX_Departments_TenantId_Name");

                    b.ToTable("Departments");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.DpaAcceptance", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("AcceptedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("AcceptedByUserId")
                        .HasColumnType("uuid");

                    b.Property<string>("CompanyRegistrationNo")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("Country")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("DpaVersion")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("IpAddress")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<string>("OrganisationLegalName")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("SignatoryFullName")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)");

                    b.Property<string>("SignatoryRole")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("AcceptedByUserId");

                    b.HasIndex("TenantId", "DpaVersion")
                        .HasDatabaseName("IX_DpaAcceptances_TenantId_DpaVersion");

                    b.ToTable("DpaAcceptances");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.Employee", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("Department")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<Guid?>("DepartmentId")
                        .HasColumnType("uuid");

                    b.Property<string>("Email")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("EmployeeCode")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<DateTime?>("EndDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("FloatLinkMethod")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<DateTime?>("FloatLinkedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int?>("FloatPersonId")
                        .HasColumnType("integer");

                    b.Property<string>("GeoTrackerID")
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<string>("JobTitle")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("Mobile")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("Notes")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<string>("Phone")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("PreferredLanguage")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)")
                        .HasDefaultValue("en");

                    b.Property<Guid?>("PrimarySiteId")
                        .HasColumnType("uuid");

                    b.Property<string>("QrPin")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<int>("QrPinFailedAttempts")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<DateTimeOffset?>("QrPinGeneratedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("QrPinIsSet")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTimeOffset?>("QrPinLastUsedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTimeOffset?>("QrPinLockedUntil")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("QrPinPlain")
                        .HasMaxLength(6)
                        .HasColumnType("character varying(6)");

                    b.Property<DateTime?>("StartDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("text");

                    b.Property<Guid?>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("DepartmentId");

                    b.HasIndex("FloatPersonId")
                        .HasDatabaseName("IX_Employees_FloatPersonId")
                        .HasFilter("\"FloatPersonId\" IS NOT NULL");

                    b.HasIndex("PrimarySiteId");

                    b.HasIndex("TenantId", "EmployeeCode")
                        .IsUnique()
                        .HasDatabaseName("IX_Employees_TenantId_EmployeeCode");

                    b.HasIndex("TenantId", "GeoTrackerID")
                        .IsUnique()
                        .HasDatabaseName("IX_Employees_TenantId_GeoTrackerID")
                        .HasFilter("\"GeoTrackerID\" IS NOT NULL");

                    b.ToTable("Employees");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.LookupCategory", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<bool>("AllowCustom")
                        .HasColumnType("boolean");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<string>("Module")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique()
                        .HasDatabaseName("IX_LookupCategories_Name");

                    b.ToTable("LookupCategories", (string)null);
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.LookupValue", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("CategoryId")
                        .HasColumnType("uuid");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<string>("Metadata")
                        .HasColumnType("jsonb");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("CategoryId", "Code")
                        .IsUnique()
                        .HasDatabaseName("IX_LookupValues_CategoryId_Code");

                    b.ToTable("LookupValues", (string)null);
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.Permission", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("Description")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<string>("Module")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("Permissions", (string)null);
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.Role", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Description")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsSystemRole")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<Guid?>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex");

                    b.ToTable("Roles", (string)null);
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.RolePermission", b =>
                {
                    b.Property<Guid>("RoleId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("PermissionId")
                        .HasColumnType("uuid");

                    b.HasKey("RoleId", "PermissionId");

                    b.HasIndex("PermissionId");

                    b.ToTable("RolePermissions", (string)null);
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.Site", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Address")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("City")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<Guid?>("CompanyId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("Email")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("FloatLinkMethod")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<DateTime?>("FloatLinkedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int?>("FloatProjectId")
                        .HasColumnType("integer");

                    b.Property<int?>("GeofenceRadiusMeters")
                        .HasColumnType("integer");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<decimal?>("Latitude")
                        .HasPrecision(10, 8)
                        .HasColumnType("numeric(10,8)");

                    b.Property<decimal?>("Longitude")
                        .HasPrecision(11, 8)
                        .HasColumnType("numeric(11,8)");

                    b.Property<string>("Notes")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<string>("Phone")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("PostalCode")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("SiteCode")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<Guid?>("SiteManagerId")
                        .HasColumnType("uuid");

                    b.Property<string>("SiteName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("CompanyId");

                    b.HasIndex("FloatProjectId")
                        .HasDatabaseName("IX_Sites_FloatProjectId")
                        .HasFilter("\"FloatProjectId\" IS NOT NULL");

                    b.HasIndex("SiteManagerId");

                    b.HasIndex("TenantId", "SiteCode")
                        .IsUnique()
                        .HasDatabaseName("IX_Sites_TenantId_SiteCode")
                        .HasFilter("\"SiteCode\" IS NOT NULL");

                    b.ToTable("Sites");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.SupervisorAssignment", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<Guid>("OperatorEmployeeId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("SupervisorEmployeeId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("OperatorEmployeeId");

                    b.HasIndex("SupervisorEmployeeId");

                    b.HasIndex("TenantId", "OperatorEmployeeId")
                        .IsUnique()
                        .HasDatabaseName("IX_SupervisorAssignments_TenantId_OperatorEmployeeId_Active")
                        .HasFilter("\"IsDeleted\" = false");

                    b.HasIndex("TenantId", "SupervisorEmployeeId", "OperatorEmployeeId")
                        .IsUnique()
                        .HasDatabaseName("IX_SupervisorAssignments_TenantId_SupervisorEmployeeId_OperatorEmployeeId");

                    b.ToTable("SupervisorAssignments");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.SystemAuditLog", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Action")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("EntityDisplayName")
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)");

                    b.Property<Guid?>("EntityId")
                        .HasColumnType("uuid");

                    b.Property<string>("EntityType")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("FailureReason")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("IpAddress")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("MetadataJson")
                        .HasColumnType("text");

                    b.Property<DateTimeOffset>("OccurredAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("Success")
                        .HasColumnType("boolean");

                    b.Property<Guid?>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("UserId")
                        .HasColumnType("uuid");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("Action")
                        .HasDatabaseName("IX_SystemAuditLogs_Action");

                    b.HasIndex("OccurredAt")
                        .HasDatabaseName("IX_SystemAuditLogs_OccurredAt");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("IX_SystemAuditLogs_TenantId");

                    b.ToTable("SystemAuditLogs", (string)null);
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.Tenant", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Code")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("CompanyName")
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)");

                    b.Property<string>("ContactEmail")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("ContactName")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasDefaultValue("Active");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("Code")
                        .IsUnique()
                        .HasDatabaseName("IX_Tenants_Code")
                        .HasFilter("\"Code\" IS NOT NULL");

                    b.HasIndex("Name")
                        .IsUnique()
                        .HasDatabaseName("IX_Tenants_Name");

                    b.ToTable("Tenants", (string)null);
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.TenantLookupValue", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("CategoryId")
                        .HasColumnType("uuid");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsEnabled")
                        .HasColumnType("boolean");

                    b.Property<Guid?>("LookupValueId")
                        .HasColumnType("uuid");

                    b.Property<string>("Metadata")
                        .HasColumnType("jsonb");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("CategoryId");

                    b.HasIndex("LookupValueId");

                    b.HasIndex("TenantId", "CategoryId", "Code")
                        .IsUnique()
                        .HasDatabaseName("IX_TenantLookupValues_TenantId_CategoryId_Code");

                    b.ToTable("TenantLookupValues", (string)null);
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.TenantModule", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<string>("ModuleName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("TenantId", "ModuleName")
                        .IsUnique()
                        .HasDatabaseName("IX_TenantModules_TenantId_ModuleName")
                        .HasFilter("\"IsDeleted\" = false");

                    b.ToTable("TenantModules");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.TenantSetting", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("Key")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Module")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("Value")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.HasKey("Id");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("IX_TenantSettings_TenantId");

                    b.HasIndex("TenantId", "Module", "Key")
                        .IsUnique()
                        .HasDatabaseName("IX_TenantSettings_Tenant_Module_Key");

                    b.ToTable("TenantSettings", (string)null);
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.User", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("integer");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Email")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("boolean");

                    b.Property<Guid?>("EmployeeId")
                        .HasColumnType("uuid");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsSuperUser")
                        .HasColumnType("boolean");

                    b.Property<DateTimeOffset?>("LastLoginAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("LastLoginIp")
                        .HasColumnType("text");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("boolean");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("text");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("text");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("boolean");

                    b.Property<string>("RefreshToken")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<DateTime?>("RefreshTokenExpiryTime")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("boolean");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasColumnType("text");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("EmployeeId");

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex");

                    b.ToTable("Users", (string)null);
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.UserRole", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("RoleId")
                        .HasColumnType("uuid");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("UserRoles", (string)null);
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.AiUsageLog", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTimeOffset>("CalledAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<int>("InputTokens")
                        .HasColumnType("integer");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsSystemCall")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("ModelId")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("OperationCategory")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<int>("OutputTokens")
                        .HasColumnType("integer");

                    b.Property<Guid?>("ReferenceEntityId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<Guid?>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_ai_usage_logs_tenant");

                    b.HasIndex("TenantId", "CalledAt")
                        .HasDatabaseName("ix_ai_usage_logs_tenant_called_at");

                    b.ToTable("AiUsageLogs", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.AiUsageSummary", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<DateOnly>("Date")
                        .HasColumnType("date");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("ModelId")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("OperationCategory")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<int>("SystemCallCount")
                        .HasColumnType("integer");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<int>("TotalCalls")
                        .HasColumnType("integer");

                    b.Property<long>("TotalInputTokens")
                        .HasColumnType("bigint");

                    b.Property<long>("TotalOutputTokens")
                        .HasColumnType("bigint");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("TenantId", "Date", "OperationCategory", "ModelId")
                        .IsUnique()
                        .HasDatabaseName("ix_ai_usage_summaries_tenant_date_category_model");

                    b.ToTable("AiUsageSummaries", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.AuditCorpus", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("CorpusId")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("character varying(30)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("Description")
                        .HasColumnType("text");

                    b.Property<Guid?>("FrozenFromPipelineVersionId")
                        .HasColumnType("uuid");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsLocked")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("LanguagePair")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTimeOffset?>("LockedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("LockedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("SectorKey")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("SignedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<Guid?>("SourceTalkId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<int>("Version")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(1);

                    b.HasKey("Id");

                    b.HasIndex("FrozenFromPipelineVersionId");

                    b.HasIndex("SourceTalkId");

                    b.HasIndex("TenantId", "CorpusId")
                        .IsUnique()
                        .HasDatabaseName("ix_audit_corpora_tenant_id_corpus_id");

                    b.ToTable("AuditCorpora", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.AuditCorpusEntry", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("CorpusId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("EntryRef")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("ExpectedOutcome")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsSafetyCritical")
                        .HasColumnType("boolean");

                    b.Property<string>("OriginalText")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("PassThreshold")
                        .HasColumnType("integer");

                    b.Property<Guid?>("PipelineVersionIdAtFreeze")
                        .HasColumnType("uuid");

                    b.Property<string>("SectionTitle")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("SectorKey")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("SourceLanguage")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<string>("TagsJson")
                        .HasColumnType("text");

                    b.Property<string>("TargetLanguage")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<string>("TranslatedText")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("CorpusId")
                        .HasDatabaseName("ix_audit_corpus_entries_corpus_id");

                    b.ToTable("AuditCorpusEntries", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.CertificateNumberCounter", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<int>("LastNumber")
                        .HasColumnType("integer");

                    b.Property<string>("Prefix")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<int>("Year")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("TenantId", "Prefix", "Year")
                        .IsUnique()
                        .HasDatabaseName("ix_certificate_number_counters_tenant_prefix_year");

                    b.ToTable("CertificateNumberCounters", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ContentCreationSession", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("AudienceRole")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Operator");

                    b.Property<string>("AuditPurpose")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("ClientName")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("DocumentRef")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("ErrorMessage")
                        .HasColumnType("text");

                    b.Property<DateTime>("ExpiresAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("IncludeQuiz")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<string>("InputMode")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<Guid?>("OutputCourseId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("OutputTalkId")
                        .HasColumnType("uuid");

                    b.Property<string>("OutputType")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("ParsedSectionsJson")
                        .HasColumnType("text");

                    b.Property<int>("PassThreshold")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(75);

                    b.Property<bool>("PreserveSourceWording")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("QuestionsJson")
                        .HasColumnType("text");

                    b.Property<string>("QuizSettingsJson")
                        .HasColumnType("text");

                    b.Property<string>("ReviewerName")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("ReviewerOrg")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("ReviewerRole")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("SectorKey")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("SettingsJson")
                        .HasColumnType("text");

                    b.Property<string>("SourceFileName")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("SourceFileType")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("SourceFileUrl")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<string>("SourceText")
                        .HasColumnType("text");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Draft");

                    b.Property<string>("SubtitleJobId")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("TargetLanguageCodes")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<string>("TranscriptText")
                        .HasColumnType("text");

                    b.Property<string>("TranscriptWordsJson")
                        .HasColumnType("text");

                    b.Property<string>("TranscriptionJobId")
                        .HasColumnType("text");

                    b.Property<string>("TranslationJobIds")
                        .HasColumnType("text");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("ValidationRunIds")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("ExpiresAt")
                        .HasDatabaseName("ix_content_creation_sessions_expires");

                    b.HasIndex("OutputCourseId");

                    b.HasIndex("OutputTalkId");

                    b.HasIndex("Status")
                        .HasDatabaseName("ix_content_creation_sessions_status");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_content_creation_sessions_tenant");

                    b.HasIndex("TenantId", "Status")
                        .HasDatabaseName("ix_content_creation_sessions_tenant_status");

                    b.ToTable("ContentCreationSessions", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.CorpusRun", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal?>("ActualCostEur")
                        .HasColumnType("decimal(10,4)");

                    b.Property<DateTimeOffset?>("CompletedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CorpusId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("ErrorMessage")
                        .HasColumnType("text");

                    b.Property<decimal?>("EstimatedCostEur")
                        .HasColumnType("decimal(10,4)");

                    b.Property<int>("FailedEntries")
                        .HasColumnType("integer");

                    b.Property<int>("FailureThresholdPercent")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(20);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsSmokeTest")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<Guid?>("LinkedPipelineChangeId")
                        .HasColumnType("uuid");

                    b.Property<int?>("MaxScoreDrop")
                        .HasColumnType("integer");

                    b.Property<decimal?>("MeanScore")
                        .HasColumnType("decimal(6,2)");

                    b.Property<int>("PassedEntries")
                        .HasColumnType("integer");

                    b.Property<Guid?>("PipelineVersionId")
                        .HasColumnType("uuid");

                    b.Property<int>("RegressionEntries")
                        .HasColumnType("integer");

                    b.Property<int>("ReviewEntries")
                        .HasColumnType("integer");

                    b.Property<int>("ScoreDropThreshold")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(10);

                    b.Property<DateTimeOffset?>("StartedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<int>("TotalEntries")
                        .HasColumnType("integer");

                    b.Property<string>("TriggerType")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("TriggeredBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("Verdict")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("LinkedPipelineChangeId");

                    b.HasIndex("PipelineVersionId");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_corpus_runs_tenant_id");

                    b.HasIndex("CorpusId", "Status")
                        .HasDatabaseName("ix_corpus_runs_corpus_id_status");

                    b.ToTable("CorpusRuns", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.CorpusRunResult", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("ArtefactsJson")
                        .HasColumnType("text");

                    b.Property<string>("BackTranslationA")
                        .HasColumnType("text");

                    b.Property<string>("BackTranslationB")
                        .HasColumnType("text");

                    b.Property<string>("BackTranslationC")
                        .HasColumnType("text");

                    b.Property<string>("BackTranslationD")
                        .HasColumnType("text");

                    b.Property<Guid>("CorpusEntryId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("CorpusRunId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<int>("EffectiveThreshold")
                        .HasColumnType("integer");

                    b.Property<string>("ExpectedOutcome")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("FinalScore")
                        .HasColumnType("integer");

                    b.Property<string>("GlossaryCorrectionsJson")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsRegression")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsSafetyCritical")
                        .HasColumnType("boolean");

                    b.Property<string>("Outcome")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("ReviewReasonsJson")
                        .HasColumnType("text");

                    b.Property<int>("RoundsUsed")
                        .HasColumnType("integer");

                    b.Property<int?>("ScoreA")
                        .HasColumnType("integer");

                    b.Property<int?>("ScoreB")
                        .HasColumnType("integer");

                    b.Property<int?>("ScoreC")
                        .HasColumnType("integer");

                    b.Property<int?>("ScoreD")
                        .HasColumnType("integer");

                    b.Property<int?>("ScoreDelta")
                        .HasColumnType("integer");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<bool>("WasCached")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.HasKey("Id");

                    b.HasIndex("CorpusEntryId");

                    b.HasIndex("CorpusRunId")
                        .HasDatabaseName("ix_corpus_run_results_corpus_run_id");

                    b.ToTable("CorpusRunResults", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.CustomerUsageReportState", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTimeOffset?>("LastReviewedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.ToTable("CustomerUsageReportStates", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.PipelineChangeRecord", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Approver")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("ChangeFrom")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("ChangeId")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("ChangeTo")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Component")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<DateTimeOffset>("DeployedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("ImpactAssessment")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("Justification")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("PipelineVersionId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("PreviousPipelineVersionId")
                        .HasColumnType("uuid");

                    b.Property<string>("PriorModulesAction")
                        .HasColumnType("text");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("text")
                        .HasDefaultValue("Draft");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("ChangeId")
                        .IsUnique()
                        .HasDatabaseName("ix_pipeline_change_records_change_id");

                    b.HasIndex("PipelineVersionId")
                        .HasDatabaseName("ix_pipeline_change_records_version");

                    b.ToTable("PipelineChangeRecords", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.PipelineVersion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("ComponentsJson")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTimeOffset>("ComputedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("Hash")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("Version")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.HasKey("Id");

                    b.HasIndex("Hash")
                        .IsUnique()
                        .HasDatabaseName("ix_pipeline_versions_hash");

                    b.HasIndex("IsActive")
                        .HasDatabaseName("ix_pipeline_versions_is_active");

                    b.ToTable("PipelineVersions", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ProviderResultCache", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("BackTranslation")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTimeOffset>("ComputedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid?>("CorpusEntryId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("Provider")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("ProviderVersion")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<int>("Score")
                        .HasColumnType("integer");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("CorpusEntryId", "Provider", "ProviderVersion")
                        .IsUnique()
                        .HasDatabaseName("ix_provider_result_cache_entry_provider_version");

                    b.ToTable("ProviderResultCache", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.QrCode", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("CodeToken")
                        .IsRequired()
                        .HasMaxLength(36)
                        .HasColumnType("character varying(36)");

                    b.Property<string>("ContentMode")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<Guid?>("CourseId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("QrImageUrl")
                        .HasMaxLength(2048)
                        .HasColumnType("character varying(2048)");

                    b.Property<Guid>("QrLocationId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ToolboxTalkId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("CodeToken")
                        .IsUnique()
                        .HasDatabaseName("ix_qr_codes_token");

                    b.HasIndex("CourseId");

                    b.HasIndex("QrLocationId");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_qr_codes_tenant");

                    b.HasIndex("ToolboxTalkId");

                    b.ToTable("QrCodes", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.QrLocation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Address")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("Description")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_qr_locations_tenant");

                    b.ToTable("QrLocations", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.QrSession", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTimeOffset?>("CompletedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("ContentMode")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<Guid>("EmployeeId")
                        .HasColumnType("uuid");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("Language")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<Guid>("QrCodeId")
                        .HasColumnType("uuid");

                    b.Property<int?>("Score")
                        .HasColumnType("integer");

                    b.Property<Guid>("SessionToken")
                        .HasColumnType("uuid");

                    b.Property<DateTimeOffset?>("SignedOffAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTimeOffset>("StartedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Active");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("EmployeeId");

                    b.HasIndex("QrCodeId");

                    b.HasIndex("SessionToken")
                        .IsUnique()
                        .HasDatabaseName("ix_qr_sessions_token");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_qr_sessions_tenant");

                    b.ToTable("QrSessions", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryBody", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("Country")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("Kind")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasDefaultValue("Regulation");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<Guid?>("SectorId")
                        .HasColumnType("uuid");

                    b.Property<string>("TranslationInstructions")
                        .HasColumnType("text");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("Website")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.HasKey("Id");

                    b.HasIndex("Code")
                        .IsUnique()
                        .HasDatabaseName("ix_regulatory_bodies_code");

                    b.HasIndex("SectorId")
                        .HasDatabaseName("ix_regulatory_bodies_sector");

                    b.ToTable("RegulatoryBodies", "toolbox_talks", t =>
                        {
                            t.HasCheckConstraint("ck_regulatory_bodies_kind_sector", "(\"Kind\" = 'Standard' AND \"SectorId\" IS NOT NULL) OR (\"Kind\" = 'Regulation' AND \"SectorId\" IS NULL)");
                        });
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryCriteria", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("CategoryKey")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("CriteriaText")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<int>("DisplayOrder")
                        .HasColumnType("integer");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<Guid>("RegulatoryProfileId")
                        .HasColumnType("uuid");

                    b.Property<string>("Source")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<Guid?>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_regulatory_criteria_tenant");

                    b.HasIndex("RegulatoryProfileId", "TenantId", "CategoryKey", "DisplayOrder")
                        .IsUnique()
                        .HasDatabaseName("ix_regulatory_criteria_profile_tenant_category_order");

                    b.ToTable("RegulatoryCriteria", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryDocument", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<DateOnly?>("EffectiveDate")
                        .HasColumnType("date");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTimeOffset?>("LastIngestedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("LastIngestionErrorCode")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("LastIngestionErrorMessage")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<string>("LastIngestionStatus")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasDefaultValue("Idle");

                    b.Property<Guid>("RegulatoryBodyId")
                        .HasColumnType("uuid");

                    b.Property<string>("Source")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("SourceUrl")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("Version")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("Id");

                    b.HasIndex("RegulatoryBodyId")
                        .HasDatabaseName("ix_regulatory_documents_body");

                    b.ToTable("RegulatoryDocuments", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryProfile", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("CategoryWeightsJson")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("ExportLabel")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<Guid>("RegulatoryDocumentId")
                        .HasColumnType("uuid");

                    b.Property<string>("ScoreLabel")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<Guid>("SectorId")
                        .HasColumnType("uuid");

                    b.Property<string>("SectorKey")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("SectorId");

                    b.HasIndex("SectorKey")
                        .HasDatabaseName("ix_regulatory_profiles_sector_key");

                    b.HasIndex("RegulatoryDocumentId", "SectorId")
                        .IsUnique()
                        .HasDatabaseName("ix_regulatory_profiles_document_sector");

                    b.ToTable("RegulatoryProfiles", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryRequirement", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Block")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<int>("DisplayOrder")
                        .HasColumnType("integer");

                    b.Property<string>("FeatureIdentifier")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("FootnoteDefinition")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<string>("IngestionNotes")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<string>("IngestionSource")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("IngestionStatus")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsProvisional")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("Principle")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("PrincipleLabel")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Priority")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<Guid>("RegulatoryProfileId")
                        .HasColumnType("uuid");

                    b.Property<string>("Section")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("SectionLabel")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("VerbatimText")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.HasKey("Id");

                    b.HasIndex("IngestionStatus")
                        .HasDatabaseName("ix_regulatory_requirements_ingestion_status");

                    b.HasIndex("RegulatoryProfileId")
                        .HasDatabaseName("ix_regulatory_requirements_profile");

                    b.ToTable("RegulatoryRequirements", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryRequirementMapping", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("AiReasoning")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<int?>("ConfidenceScore")
                        .HasColumnType("integer");

                    b.Property<Guid?>("CourseId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("MappingStatus")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<Guid>("RegulatoryRequirementId")
                        .HasColumnType("uuid");

                    b.Property<string>("ReviewNotes")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<DateTimeOffset?>("ReviewedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("ReviewedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ToolboxTalkId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("CourseId");

                    b.HasIndex("RegulatoryRequirementId");

                    b.HasIndex("ToolboxTalkId");

                    b.HasIndex("TenantId", "MappingStatus")
                        .HasDatabaseName("ix_regulatory_requirement_mappings_tenant_status");

                    b.HasIndex("TenantId", "RegulatoryRequirementId", "CourseId")
                        .IsUnique()
                        .HasDatabaseName("ix_regulatory_requirement_mappings_tenant_req_course")
                        .HasFilter("\"CourseId\" IS NOT NULL");

                    b.HasIndex("TenantId", "RegulatoryRequirementId", "ToolboxTalkId")
                        .IsUnique()
                        .HasDatabaseName("ix_regulatory_requirement_mappings_tenant_req_talk")
                        .HasFilter("\"ToolboxTalkId\" IS NOT NULL");

                    b.ToTable("RegulatoryRequirementMappings", "toolbox_talks", t =>
                        {
                            t.HasCheckConstraint("ck_regulatory_requirement_mappings_talk_or_course", "(\"ToolboxTalkId\" IS NOT NULL AND \"CourseId\" IS NULL) OR (\"ToolboxTalkId\" IS NULL AND \"CourseId\" IS NOT NULL)");
                        });
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryStructureMap", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<Guid>("RegulatoryDocumentId")
                        .HasColumnType("uuid");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasDefaultValue("Draft");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<DateTimeOffset?>("VerifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("VerifiedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("RegulatoryDocumentId")
                        .IsUnique()
                        .HasDatabaseName("ix_regulatory_structure_maps_document");

                    b.ToTable("RegulatoryStructureMaps", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryStructureMapFeature", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Block")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<int>("DisplayOrder")
                        .HasColumnType("integer");

                    b.Property<string>("FootnoteDefinition")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<string>("Identifier")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<Guid>("RegulatoryStructureMapStandardId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("VerbatimText")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.HasKey("Id");

                    b.HasIndex("RegulatoryStructureMapStandardId", "Block", "Identifier")
                        .IsUnique()
                        .HasDatabaseName("ix_regulatory_structure_map_features_standard_block_identifier");

                    b.ToTable("RegulatoryStructureMapFeatures", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryStructureMapPrinciple", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<int>("DisplayOrder")
                        .HasColumnType("integer");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<int>("Number")
                        .HasColumnType("integer");

                    b.Property<Guid>("RegulatoryStructureMapId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("RegulatoryStructureMapId", "Number")
                        .IsUnique()
                        .HasDatabaseName("ix_regulatory_structure_map_principles_map_number");

                    b.ToTable("RegulatoryStructureMapPrinciples", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryStructureMapStandard", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<int>("DisplayOrder")
                        .HasColumnType("integer");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<Guid>("RegulatoryStructureMapPrincipleId")
                        .HasColumnType("uuid");

                    b.Property<string>("StandardId")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("RegulatoryStructureMapPrincipleId", "StandardId")
                        .IsUnique()
                        .HasDatabaseName("ix_regulatory_structure_map_standards_principle_standard");

                    b.ToTable("RegulatoryStructureMapStandards", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.SafetyGlossary", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("SectorIcon")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("SectorKey")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("SectorName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<Guid?>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_safety_glossaries_tenant");

                    b.HasIndex("TenantId", "SectorKey")
                        .IsUnique()
                        .HasDatabaseName("ix_safety_glossaries_tenant_sector");

                    b.ToTable("SafetyGlossaries", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.SafetyGlossaryTerm", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("EnglishTerm")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<Guid>("GlossaryId")
                        .HasColumnType("uuid");

                    b.Property<bool>("IsCritical")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("Translations")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("text")
                        .HasDefaultValue("{}");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("Category")
                        .HasDatabaseName("ix_safety_glossary_terms_category");

                    b.HasIndex("GlossaryId")
                        .HasDatabaseName("ix_safety_glossary_terms_glossary");

                    b.HasIndex("GlossaryId", "EnglishTerm")
                        .IsUnique()
                        .HasDatabaseName("ix_safety_glossary_terms_glossary_term");

                    b.ToTable("SafetyGlossaryTerms", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ScheduledTalk", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CourseAssignmentId")
                        .HasColumnType("uuid");

                    b.Property<int?>("CourseOrderIndex")
                        .HasColumnType("integer");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<DateTime>("DueDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("EmployeeId")
                        .HasColumnType("uuid");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsRefresher")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)")
                        .HasDefaultValue("en");

                    b.Property<DateTime?>("LastReminderAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid?>("OriginalScheduledTalkId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("RefresherDueDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("ReminderSent1Week")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("ReminderSent2Weeks")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<int>("RemindersSent")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<DateTime>("RequiredDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid?>("ScheduleId")
                        .HasColumnType("uuid");

                    b.Property<double?>("StartedAccuracyMeters")
                        .HasPrecision(10, 2)
                        .HasColumnType("double precision");

                    b.Property<double?>("StartedLatitude")
                        .HasPrecision(9, 6)
                        .HasColumnType("double precision");

                    b.Property<DateTime?>("StartedLocationTimestamp")
                        .HasColumnType("timestamp with time zone");

                    b.Property<double?>("StartedLongitude")
                        .HasPrecision(9, 6)
                        .HasColumnType("double precision");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Pending");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("ToolboxTalkId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<int>("VideoWatchPercent")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("CourseAssignmentId")
                        .HasDatabaseName("ix_scheduled_talks_course_assignment");

                    b.HasIndex("DueDate")
                        .HasDatabaseName("ix_scheduled_talks_due_date");

                    b.HasIndex("EmployeeId")
                        .HasDatabaseName("ix_scheduled_talks_employee");

                    b.HasIndex("OriginalScheduledTalkId")
                        .HasDatabaseName("ix_scheduled_talks_original");

                    b.HasIndex("ScheduleId")
                        .HasDatabaseName("ix_scheduled_talks_schedule");

                    b.HasIndex("Status")
                        .HasDatabaseName("ix_scheduled_talks_status");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_scheduled_talks_tenant");

                    b.HasIndex("ToolboxTalkId")
                        .HasDatabaseName("ix_scheduled_talks_talk");

                    b.HasIndex("TenantId", "EmployeeId", "Status")
                        .HasDatabaseName("ix_scheduled_talks_tenant_employee_status");

                    b.ToTable("ScheduledTalks", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ScheduledTalkCompletion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<bool>("CertificateGenerationFailed")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("CertificateUrl")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<double?>("CompletedAccuracyMeters")
                        .HasPrecision(10, 2)
                        .HasColumnType("double precision");

                    b.Property<DateTime>("CompletedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<double?>("CompletedLatitude")
                        .HasPrecision(9, 6)
                        .HasColumnType("double precision");

                    b.Property<DateTime?>("CompletedLocationTimestamp")
                        .HasColumnType("timestamp with time zone");

                    b.Property<double?>("CompletedLongitude")
                        .HasPrecision(9, 6)
                        .HasColumnType("double precision");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("IPAddress")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<int?>("QuizMaxScore")
                        .HasColumnType("integer");

                    b.Property<bool?>("QuizPassed")
                        .HasColumnType("boolean");

                    b.Property<int?>("QuizScore")
                        .HasColumnType("integer");

                    b.Property<Guid>("ScheduledTalkId")
                        .HasColumnType("uuid");

                    b.Property<string>("SignatureData")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("SignedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("SignedByName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<int>("TotalTimeSpentSeconds")
                        .HasColumnType("integer");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("UserAgent")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<int?>("VideoWatchPercent")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("CompletedAt")
                        .HasDatabaseName("ix_scheduled_talk_completions_completed_at");

                    b.HasIndex("ScheduledTalkId")
                        .IsUnique()
                        .HasDatabaseName("ix_scheduled_talk_completions_talk");

                    b.ToTable("ScheduledTalkCompletions", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ScheduledTalkQuizAttempt", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Answers")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("text")
                        .HasDefaultValue("{}");

                    b.Property<int>("AttemptNumber")
                        .HasColumnType("integer");

                    b.Property<DateTime>("AttemptedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("GeneratedQuestionsJson")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<int>("MaxScore")
                        .HasColumnType("integer");

                    b.Property<bool>("Passed")
                        .HasColumnType("boolean");

                    b.Property<decimal>("Percentage")
                        .HasPrecision(5, 2)
                        .HasColumnType("numeric(5,2)");

                    b.Property<Guid>("ScheduledTalkId")
                        .HasColumnType("uuid");

                    b.Property<int>("Score")
                        .HasColumnType("integer");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("ScheduledTalkId")
                        .HasDatabaseName("ix_scheduled_talk_quiz_attempts_talk");

                    b.HasIndex("ScheduledTalkId", "AttemptNumber")
                        .IsUnique()
                        .HasDatabaseName("ix_scheduled_talk_quiz_attempts_talk_attempt");

                    b.ToTable("ScheduledTalkQuizAttempts", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ScheduledTalkSectionProgress", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsRead")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTime?>("ReadAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ScheduledTalkId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("SectionId")
                        .HasColumnType("uuid");

                    b.Property<int>("TimeSpentSeconds")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("ScheduledTalkId")
                        .HasDatabaseName("ix_scheduled_talk_section_progress_talk");

                    b.HasIndex("SectionId")
                        .HasDatabaseName("ix_scheduled_talk_section_progress_section");

                    b.HasIndex("ScheduledTalkId", "SectionId")
                        .IsUnique()
                        .HasDatabaseName("ix_scheduled_talk_section_progress_talk_section");

                    b.ToTable("ScheduledTalkSectionProgress", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.Sector", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<int>("DisplayOrder")
                        .HasColumnType("integer");

                    b.Property<string>("Icon")
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("Key")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("Key")
                        .IsUnique()
                        .HasDatabaseName("ix_sectors_key");

                    b.ToTable("Sectors", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.SubtitleProcessingJob", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("CachedTranscriptWordsJson")
                        .HasColumnType("text");

                    b.Property<DateTime?>("CompletedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("EnglishSrtContent")
                        .HasColumnType("text");

                    b.Property<string>("EnglishSrtUrl")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("ErrorMessage")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("SourceVideoUrl")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<DateTime?>("StartedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Pending");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("ToolboxTalkId")
                        .HasColumnType("uuid");

                    b.Property<int>("TotalSubtitles")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("VideoSourceType")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("Id");

                    b.HasIndex("Status")
                        .HasDatabaseName("ix_subtitle_processing_jobs_status");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_subtitle_processing_jobs_tenant");

                    b.HasIndex("ToolboxTalkId")
                        .HasDatabaseName("ix_subtitle_processing_jobs_toolbox_talk");

                    b.HasIndex("TenantId", "Status")
                        .HasDatabaseName("ix_subtitle_processing_jobs_tenant_status");

                    b.HasIndex("TenantId", "ToolboxTalkId", "IsDeleted")
                        .HasDatabaseName("ix_subtitle_processing_jobs_tenant_talk_deleted");

                    b.ToTable("SubtitleProcessingJobs", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.SubtitleTranslation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("ErrorMessage")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("Language")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<string>("SrtContent")
                        .HasColumnType("text");

                    b.Property<string>("SrtUrl")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Pending");

                    b.Property<Guid>("SubtitleProcessingJobId")
                        .HasColumnType("uuid");

                    b.Property<int>("SubtitlesProcessed")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<int>("TotalSubtitles")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("Status")
                        .HasDatabaseName("ix_subtitle_translations_status");

                    b.HasIndex("SubtitleProcessingJobId")
                        .HasDatabaseName("ix_subtitle_translations_job");

                    b.HasIndex("SubtitleProcessingJobId", "LanguageCode")
                        .IsUnique()
                        .HasDatabaseName("ix_subtitle_translations_job_language");

                    b.ToTable("SubtitleTranslations", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TenantReviewerConfiguration", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("LanguageCode")
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<string>("ReviewerEmail")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("ReviewerName")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("TenantId")
                        .IsUnique()
                        .HasDatabaseName("ix_tenant_reviewer_configurations_tenant_fallback")
                        .HasFilter("\"IsDeleted\" = false AND \"LanguageCode\" IS NULL");

                    b.HasIndex("TenantId", "LanguageCode")
                        .IsUnique()
                        .HasDatabaseName("ix_tenant_reviewer_configurations_tenant_language")
                        .HasFilter("\"IsDeleted\" = false AND \"LanguageCode\" IS NOT NULL");

                    b.ToTable("TenantReviewerConfigurations", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TenantSector", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDefault")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<Guid>("SectorId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("SectorId");

                    b.HasIndex("TenantId", "SectorId")
                        .IsUnique()
                        .HasDatabaseName("ix_tenant_sectors_tenant_sector");

                    b.ToTable("TenantSectors", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TenantStandardSubscription", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<Guid>("RegulatoryBodyId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("RegulatoryBodyId");

                    b.HasIndex("TenantId", "RegulatoryBodyId")
                        .IsUnique()
                        .HasDatabaseName("ix_tenant_standard_subscriptions_tenant_body");

                    b.ToTable("TenantStandardSubscriptions", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<bool>("AllowRetry")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<string>("AttachmentUrl")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("AudienceRole")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("AuditPurpose")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<int>("AutoAssignDueDays")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(14);

                    b.Property<bool>("AutoAssignToNewEmployees")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTimeOffset?>("BulkTranslationPendingSince")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Category")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("ClientName")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime?>("ContentGeneratedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CoverImageUrl")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("Description")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<string>("DocumentRef")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("ExtractedPdfText")
                        .HasColumnType("text");

                    b.Property<string>("ExtractedVideoTranscript")
                        .HasColumnType("text");

                    b.Property<string>("Frequency")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Once");

                    b.Property<bool>("GenerateCertificate")
                        .HasColumnType("boolean");

                    b.Property<bool>("GenerateSlidesFromPdf")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("GeneratedFromPdf")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("GeneratedFromVideo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("InputMode")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasDefaultValue("Text");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsPartOfCourse")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<int?>("LastEditedStep")
                        .HasColumnType("integer");

                    b.Property<int>("MinimumVideoWatchPercent")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(90);

                    b.Property<int?>("PassingScore")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(80);

                    b.Property<string>("PdfFileHash")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)");

                    b.Property<string>("PdfFileName")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<DateTime?>("PdfTextExtractedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("PdfUrl")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<bool>("PreserveSourceWording")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTime?>("PublishedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int?>("QuizQuestionCount")
                        .HasColumnType("integer");

                    b.Property<int>("RefresherIntervalMonths")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(12);

                    b.Property<bool>("RequiresQuiz")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("RequiresRefresher")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("ReviewerName")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("ReviewerOrg")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("ReviewerRole")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<bool>("ShuffleOptions")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("ShuffleQuestions")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("SlidesGenerated")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTime?>("SlideshowGeneratedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("SlideshowHtml")
                        .HasColumnType("text");

                    b.Property<string>("SourceFileName")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("SourceFileType")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("SourceFileUrl")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("SourceLanguageCode")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)")
                        .HasDefaultValue("en");

                    b.Property<string>("SourceText")
                        .HasColumnType("text");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Draft");

                    b.Property<string>("TargetLanguageCodes")
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("TranscriptWordsJson")
                        .HasColumnType("text");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<bool>("UseQuestionPool")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("VideoFileHash")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)");

                    b.Property<string>("VideoSource")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("None");

                    b.Property<DateTime?>("VideoTranscriptExtractedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("VideoUrl")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.HasKey("Id");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_toolbox_talks_tenant");

                    b.HasIndex("Title")
                        .HasDatabaseName("ix_toolbox_talks_title");

                    b.HasIndex("TenantId", "BulkTranslationPendingSince")
                        .HasDatabaseName("ix_toolbox_talks_tenant_bulk_translation_pending")
                        .HasFilter("\"BulkTranslationPendingSince\" IS NOT NULL");

                    b.HasIndex("TenantId", "Code")
                        .IsUnique()
                        .HasDatabaseName("IX_ToolboxTalks_TenantId_Code")
                        .HasFilter("\"IsDeleted\" = false");

                    b.HasIndex("TenantId", "PdfFileHash")
                        .HasDatabaseName("ix_toolbox_talks_tenant_pdf_hash")
                        .HasFilter("\"PdfFileHash\" IS NOT NULL");

                    b.HasIndex("TenantId", "VideoFileHash")
                        .HasDatabaseName("ix_toolbox_talks_tenant_video_hash")
                        .HasFilter("\"VideoFileHash\" IS NOT NULL");

                    b.HasIndex("TenantId", "IsDeleted", "IsActive")
                        .HasDatabaseName("ix_toolbox_talks_tenant_deleted_active");

                    b.ToTable("ToolboxTalks", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCertificate", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<bool>("CertificateEmailFailed")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("CertificateNumber")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("CertificateType")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<Guid?>("CourseAssignmentId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CourseId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("EmployeeCode")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<Guid>("EmployeeId")
                        .HasColumnType("uuid");

                    b.Property<string>("EmployeeName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<DateTime?>("ExpiresAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("IncludedTalksJson")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsRefresher")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTime>("IssuedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("LearningCode")
                        .HasColumnType("text");

                    b.Property<string>("PdfStoragePath")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<Guid?>("ScheduledTalkId")
                        .HasColumnType("uuid");

                    b.Property<string>("SignatureDataUrl")
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ToolboxTalkId")
                        .HasColumnType("uuid");

                    b.Property<string>("TrainingTitle")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("CourseId");

                    b.HasIndex("EmployeeId")
                        .HasDatabaseName("ix_toolbox_talk_certificates_employee");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_toolbox_talk_certificates_tenant");

                    b.HasIndex("ToolboxTalkId");

                    b.HasIndex("TenantId", "CertificateNumber")
                        .IsUnique()
                        .HasDatabaseName("ix_toolbox_talk_certificates_number");

                    b.HasIndex("TenantId", "EmployeeId")
                        .HasDatabaseName("ix_toolbox_talk_certificates_tenant_employee");

                    b.ToTable("ToolboxTalkCertificates", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourse", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<int>("AutoAssignDueDays")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(14);

                    b.Property<bool>("AutoAssignToNewEmployees")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("Description")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<bool>("GenerateCertificate")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<int>("RefresherIntervalMonths")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(12);

                    b.Property<bool>("RequireSequentialCompletion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("RequiresRefresher")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_toolbox_talk_courses_tenant");

                    b.HasIndex("TenantId", "IsActive")
                        .HasDatabaseName("ix_toolbox_talk_courses_tenant_active");

                    b.ToTable("ToolboxTalkCourses", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourseAssignment", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("AssignedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("CertificateGenerationFailed")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTime?>("CompletedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CourseId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<DateTime?>("DueDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("EmployeeId")
                        .HasColumnType("uuid");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsRefresher")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<Guid?>("OriginalAssignmentId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("RefresherDueDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("ReminderSent1Week")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("ReminderSent2Weeks")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTime?>("StartedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasDefaultValue("Assigned");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("CourseId")
                        .HasDatabaseName("ix_course_assignments_course");

                    b.HasIndex("EmployeeId")
                        .HasDatabaseName("ix_course_assignments_employee");

                    b.HasIndex("OriginalAssignmentId");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_course_assignments_tenant");

                    b.HasIndex("CourseId", "EmployeeId")
                        .HasDatabaseName("ix_course_assignments_course_employee");

                    b.HasIndex("TenantId", "Status")
                        .HasDatabaseName("ix_course_assignments_tenant_status");

                    b.ToTable("ToolboxTalkCourseAssignments", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourseItem", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("CourseId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsRequired")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<int>("OrderIndex")
                        .HasColumnType("integer");

                    b.Property<Guid>("ToolboxTalkId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("CourseId")
                        .HasDatabaseName("ix_toolbox_talk_course_items_course");

                    b.HasIndex("ToolboxTalkId");

                    b.HasIndex("CourseId", "OrderIndex")
                        .HasDatabaseName("ix_toolbox_talk_course_items_course_order");

                    b.HasIndex("CourseId", "ToolboxTalkId")
                        .IsUnique()
                        .HasDatabaseName("ix_toolbox_talk_course_items_course_talk");

                    b.ToTable("ToolboxTalkCourseItems", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourseTranslation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("CourseId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<string>("TranslatedDescription")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<string>("TranslatedTitle")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("CourseId")
                        .HasDatabaseName("ix_toolbox_talk_course_translations_course");

                    b.HasIndex("CourseId", "LanguageCode")
                        .IsUnique()
                        .HasDatabaseName("ix_toolbox_talk_course_translations_course_language");

                    b.ToTable("ToolboxTalkCourseTranslations", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkQuestion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("CorrectAnswer")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<int?>("CorrectOptionIndex")
                        .HasColumnType("integer");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsFromVideoFinalPortion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("Options")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<int>("Points")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(1);

                    b.Property<int>("QuestionNumber")
                        .HasColumnType("integer");

                    b.Property<string>("QuestionText")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<string>("QuestionType")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("MultipleChoice");

                    b.Property<string>("Source")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Manual");

                    b.Property<Guid>("ToolboxTalkId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("VideoTimestamp")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("Id");

                    b.HasIndex("ToolboxTalkId")
                        .HasDatabaseName("ix_toolbox_talk_questions_talk");

                    b.HasIndex("ToolboxTalkId", "QuestionNumber")
                        .HasDatabaseName("ix_toolbox_talk_questions_talk_number");

                    b.ToTable("ToolboxTalkQuestions", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSchedule", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<bool>("AssignToAllEmployees")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<DateTime?>("EndDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Frequency")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Once");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTime?>("LastProcessedCycleDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("NextRunDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Notes")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<DateTime>("ScheduledDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Draft");

                    b.PrimitiveCollection<List<Guid>>("TargetDepartmentIds")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid[]")
                        .HasDefaultValueSql("'{}'");

                    b.PrimitiveCollection<List<Guid>>("TargetSiteIds")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid[]")
                        .HasDefaultValueSql("'{}'");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("ToolboxTalkId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("NextRunDate")
                        .HasDatabaseName("ix_toolbox_talk_schedules_next_run");

                    b.HasIndex("Status")
                        .HasDatabaseName("ix_toolbox_talk_schedules_status");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_toolbox_talk_schedules_tenant");

                    b.HasIndex("ToolboxTalkId")
                        .HasDatabaseName("ix_toolbox_talk_schedules_talk");

                    b.HasIndex("TenantId", "Status", "ScheduledDate")
                        .HasDatabaseName("ix_toolbox_talk_schedules_tenant_status_date");

                    b.ToTable("ToolboxTalkSchedules", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkScheduleAssignment", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<Guid>("EmployeeId")
                        .HasColumnType("uuid");

                    b.Property<bool>("IsCriteriaDerived")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsProcessed")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTime?>("ProcessedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ScheduleId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("EmployeeId")
                        .HasDatabaseName("ix_toolbox_talk_schedule_assignments_employee");

                    b.HasIndex("ScheduleId")
                        .HasDatabaseName("ix_toolbox_talk_schedule_assignments_schedule");

                    b.HasIndex("ScheduleId", "EmployeeId")
                        .IsUnique()
                        .HasDatabaseName("ix_toolbox_talk_schedule_assignments_schedule_employee");

                    b.ToTable("ToolboxTalkScheduleAssignments", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSection", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("RequiresAcknowledgment")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<int>("SectionNumber")
                        .HasColumnType("integer");

                    b.Property<string>("Source")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Manual");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<Guid>("ToolboxTalkId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("VideoTimestamp")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("Id");

                    b.HasIndex("ToolboxTalkId")
                        .HasDatabaseName("ix_toolbox_talk_sections_talk");

                    b.HasIndex("ToolboxTalkId", "SectionNumber")
                        .HasDatabaseName("ix_toolbox_talk_sections_talk_number");

                    b.ToTable("ToolboxTalkSections", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSettings", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<bool>("DefaultAllowRetry")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("DefaultAutoAssign")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<int>("DefaultAutoAssignDueDays")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(14);

                    b.Property<int>("DefaultDueDays")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(7);

                    b.Property<bool>("DefaultGenerateCertificate")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("DefaultGenerateSlideshow")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("DefaultIncludeQuiz")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("DefaultIsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<int>("DefaultMinimumVideoWatchPercent")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(90);

                    b.Property<int>("DefaultPassingScore")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(80);

                    b.Property<bool>("DefaultPreserveSourceWording")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<string>("DefaultRefresherFrequency")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasDefaultValue("Once");

                    b.Property<bool>("DefaultShuffleOptions")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("DefaultShuffleQuestions")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("DefaultUseQuestionPool")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("DefaultVideoRightsConfirmed")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("EnableTranslation")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("EnableVideoDubbing")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<int>("EscalateAfterReminders")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(3);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<int>("MaxReminders")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(5);

                    b.Property<string>("NotificationEmailTemplate")
                        .HasColumnType("text");

                    b.Property<bool>("NotifyOnExternalReviewResponse")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("NotifyOnFailure")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("NotifyOnTranslationComplete")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<bool>("NotifyOnValidationComplete")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<string>("ReminderEmailTemplate")
                        .HasColumnType("text");

                    b.Property<int>("ReminderFrequencyDays")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(1);

                    b.Property<bool>("RequireVideoCompletion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<string>("TranslationProvider")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("VideoDubbingProvider")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("Id");

                    b.HasIndex("TenantId")
                        .IsUnique()
                        .HasDatabaseName("ix_toolbox_talk_settings_tenant");

                    b.ToTable("ToolboxTalkSettings", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSlide", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("ImageStoragePath")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("OriginalText")
                        .HasColumnType("text");

                    b.Property<int>("PageNumber")
                        .HasColumnType("integer");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("ToolboxTalkId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("ToolboxTalkId")
                        .HasDatabaseName("ix_toolbox_talk_slides_talk");

                    b.HasIndex("ToolboxTalkId", "PageNumber")
                        .IsUnique()
                        .HasDatabaseName("ix_toolbox_talk_slides_talk_page")
                        .HasFilter("\"IsDeleted\" = false");

                    b.ToTable("ToolboxTalkSlides", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSlideTranslation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<Guid>("SlideId")
                        .HasColumnType("uuid");

                    b.Property<string>("TranslatedText")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("SlideId", "LanguageCode")
                        .IsUnique()
                        .HasDatabaseName("ix_toolbox_talk_slide_translations_slide_language");

                    b.ToTable("ToolboxTalkSlideTranslations", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSlideshowTranslation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<Guid>("ToolboxTalkId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("TranslatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("TranslatedHtml")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("ToolboxTalkId", "LanguageCode")
                        .IsUnique()
                        .HasDatabaseName("ix_toolbox_talk_slideshow_translations_talk_language");

                    b.ToTable("ToolboxTalkSlideshowTranslations", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkTranslation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("EmailBody")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("EmailSubject")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<DateTime?>("LastExternalReviewedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("LastExternalReviewedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<bool>("NeedsRevalidation")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("ToolboxTalkId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("TranslatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("TranslatedDescription")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<string>("TranslatedQuestions")
                        .HasColumnType("text");

                    b.Property<string>("TranslatedSections")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("text")
                        .HasDefaultValue("[]");

                    b.Property<string>("TranslatedTitle")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("TranslationProvider")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_toolbox_talk_translations_tenant");

                    b.HasIndex("ToolboxTalkId")
                        .HasDatabaseName("ix_toolbox_talk_translations_talk");

                    b.HasIndex("ToolboxTalkId", "LanguageCode")
                        .IsUnique()
                        .HasDatabaseName("ix_toolbox_talk_translations_talk_language");

                    b.ToTable("ToolboxTalkTranslations", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkVideoTranslation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("CompletedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("ErrorMessage")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<string>("ExternalProjectId")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<string>("OriginalVideoUrl")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Pending");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("ToolboxTalkId")
                        .HasColumnType("uuid");

                    b.Property<string>("TranslatedVideoUrl")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("Status")
                        .HasDatabaseName("ix_toolbox_talk_video_translations_status");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_toolbox_talk_video_translations_tenant");

                    b.HasIndex("ToolboxTalkId")
                        .HasDatabaseName("ix_toolbox_talk_video_translations_talk");

                    b.HasIndex("ToolboxTalkId", "LanguageCode")
                        .IsUnique()
                        .HasDatabaseName("ix_toolbox_talk_video_translations_talk_language");

                    b.ToTable("ToolboxTalkVideoTranslations", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TranslationDeviation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Approver")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<DateTimeOffset?>("ClosedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("ClosedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("CorrectiveAction")
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<DateTimeOffset>("DetectedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("DetectedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeviationId")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("LanguagePair")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("LessonRef")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("ModuleRef")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("Nature")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("PipelineVersionAtTime")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("PreventiveAction")
                        .HasColumnType("text");

                    b.Property<string>("RootCauseCategory")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("RootCauseDetail")
                        .HasColumnType("text");

                    b.Property<string>("SourceExcerpt")
                        .HasColumnType("text");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("TargetExcerpt")
                        .HasColumnType("text");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<Guid?>("ValidationResultId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ValidationRunId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("ValidationRunId");

                    b.HasIndex("TenantId", "DeviationId")
                        .IsUnique()
                        .HasDatabaseName("ix_translation_deviations_tenant_deviation_id");

                    b.HasIndex("TenantId", "ValidationRunId")
                        .HasDatabaseName("ix_translation_deviations_tenant_run");

                    b.HasIndex("TenantId", "Status", "DetectedAt")
                        .HasDatabaseName("ix_translation_deviations_tenant_status_detected");

                    b.ToTable("TranslationDeviations", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TranslationFlag", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<int>("EndOffset")
                        .HasColumnType("integer");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasMaxLength(8)
                        .HasColumnType("character varying(8)");

                    b.Property<string>("Reason")
                        .IsRequired()
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)");

                    b.Property<int>("Severity")
                        .HasColumnType("integer");

                    b.Property<int>("StartOffset")
                        .HasColumnType("integer");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("ToolboxTalkId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<Guid>("ValidationResultId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_translation_flags_tenant");

                    b.HasIndex("ValidationResultId")
                        .HasDatabaseName("ix_translation_flags_validation_result");

                    b.HasIndex("ToolboxTalkId", "LanguageCode")
                        .HasDatabaseName("ix_translation_flags_talk_language");

                    b.ToTable("TranslationFlags", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TranslationValidationResult", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("ArtefactsJson")
                        .HasColumnType("text");

                    b.Property<string>("BackTranslationA")
                        .HasColumnType("text");

                    b.Property<string>("BackTranslationB")
                        .HasColumnType("text");

                    b.Property<string>("BackTranslationC")
                        .HasColumnType("text");

                    b.Property<string>("BackTranslationD")
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("CriticalTerms")
                        .HasColumnType("text");

                    b.Property<DateTime?>("DecisionAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("DecisionBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("EditedSource")
                        .HasColumnType("text");

                    b.Property<string>("EditedTranslation")
                        .HasColumnType("text");

                    b.Property<int>("EffectiveThreshold")
                        .HasColumnType("integer");

                    b.Property<string>("EngineOutcome")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<int>("FinalScore")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<string>("GlossaryCorrectionsJson")
                        .HasColumnType("text");

                    b.Property<bool?>("GlossaryHardBlockApplied")
                        .HasColumnType("boolean");

                    b.Property<string>("GlossaryMismatches")
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsSafetyCritical")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("OriginalText")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Outcome")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("RegistryViolationsJson")
                        .HasColumnType("text");

                    b.Property<string>("ReviewReasonsJson")
                        .HasColumnType("text");

                    b.Property<string>("ReviewerDecision")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Pending");

                    b.Property<int>("RoundsUsed")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(1);

                    b.Property<int>("ScoreA")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<int>("ScoreB")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<int?>("ScoreC")
                        .HasColumnType("integer");

                    b.Property<int?>("ScoreD")
                        .HasColumnType("integer");

                    b.Property<int>("SectionIndex")
                        .HasColumnType("integer");

                    b.Property<string>("SectionTitle")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("TranslatedText")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<Guid>("ValidationRunId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("Outcome")
                        .HasDatabaseName("ix_translation_validation_results_outcome");

                    b.HasIndex("ReviewerDecision")
                        .HasDatabaseName("ix_translation_validation_results_decision");

                    b.HasIndex("ValidationRunId")
                        .HasDatabaseName("ix_translation_validation_results_run");

                    b.HasIndex("ValidationRunId", "SectionIndex")
                        .IsUnique()
                        .HasDatabaseName("ix_translation_validation_results_run_section");

                    b.ToTable("TranslationValidationResults", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TranslationValidationRun", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("AuditPurpose")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("AuditReportUrl")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("ClientName")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<DateTime?>("CompletedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid?>("CourseId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("DocumentRef")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<int>("FailedSections")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsNewWizard")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<string>("OverallOutcome")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<int>("OverallScore")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<int>("PassThreshold")
                        .HasColumnType("integer");

                    b.Property<int>("PassedSections")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<Guid?>("PipelineVersionId")
                        .HasColumnType("uuid");

                    b.Property<string>("PreFlightScanJson")
                        .HasColumnType("text");

                    b.Property<int>("ReviewSections")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<string>("ReviewerName")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("ReviewerOrg")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("ReviewerRole")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("SafetyVerdict")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("SectorKey")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("SourceDialect")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("SourceLanguage")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<DateTime?>("StartedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Pending");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ToolboxTalkId")
                        .HasColumnType("uuid");

                    b.Property<int>("TotalSections")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("CourseId")
                        .HasDatabaseName("ix_translation_validation_runs_course");

                    b.HasIndex("PipelineVersionId")
                        .HasDatabaseName("ix_translation_validation_runs_pipeline_version");

                    b.HasIndex("Status")
                        .HasDatabaseName("ix_translation_validation_runs_status");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_translation_validation_runs_tenant");

                    b.HasIndex("ToolboxTalkId")
                        .HasDatabaseName("ix_translation_validation_runs_talk");

                    b.HasIndex("TenantId", "CourseId", "LanguageCode")
                        .HasDatabaseName("ix_translation_validation_runs_tenant_course_lang");

                    b.HasIndex("TenantId", "ToolboxTalkId", "LanguageCode")
                        .HasDatabaseName("ix_translation_validation_runs_tenant_talk_lang");

                    b.ToTable("TranslationValidationRuns", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ValidationRegulatoryScore", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("CategoryScoresJson")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("FullResponseJson")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<int>("OverallScore")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<string>("RegulatoryBody")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<Guid?>("RegulatoryProfileId")
                        .HasColumnType("uuid");

                    b.Property<string>("RunLabel")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<int>("RunNumber")
                        .HasColumnType("integer");

                    b.Property<string>("ScoreType")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<int>("ScoredSectionCount")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<string>("Summary")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)");

                    b.Property<string>("TargetLanguage")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<Guid>("ValidationRunId")
                        .HasColumnType("uuid");

                    b.Property<string>("Verdict")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.HasKey("Id");

                    b.HasIndex("RegulatoryProfileId")
                        .HasDatabaseName("ix_validation_regulatory_scores_regulatory_profile");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_validation_regulatory_scores_tenant");

                    b.HasIndex("ValidationRunId")
                        .HasDatabaseName("ix_validation_regulatory_scores_run");

                    b.HasIndex("ValidationRunId", "ScoreType")
                        .HasDatabaseName("ix_validation_regulatory_scores_run_type");

                    b.ToTable("ValidationRegulatoryScores", "toolbox_talks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.Workflows.ExternalParticipantInvitation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("ContextPayload")
                        .HasColumnType("text");

                    b.Property<string>("ContextType")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("EditableSectionIndicesJson")
                        .HasColumnType("text");

                    b.Property<DateTime>("ExpiresAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime>("InvitedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("InvitedEmail")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<Guid>("RequesterUserId")
                        .HasColumnType("uuid");

                    b.Property<int>("Status")
                        .HasColumnType("integer");

                    b.Property<Guid>("TargetEntityId")
                        .HasColumnType("uuid");

                    b.Property<string>("TargetEntitySubKey")
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<string>("TokenHash")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<DateTime?>("UsedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("WorkflowType")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_external_participant_invitations_tenant");

                    b.HasIndex("TokenHash")
                        .IsUnique()
                        .HasDatabaseName("ix_external_participant_invitations_token_hash");

                    b.HasIndex("WorkflowType", "TargetEntityId", "TargetEntitySubKey")
                        .HasDatabaseName("ix_external_participant_invitations_target");

                    b.ToTable("ExternalParticipantInvitations", "workflows");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.Workflows.WorkflowEvent", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("EventType")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<DateTime>("OccurredAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("PayloadJson")
                        .HasColumnType("jsonb");

                    b.Property<Guid>("TargetEntityId")
                        .HasColumnType("uuid");

                    b.Property<string>("TargetEntitySubKey")
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<int>("TriggeredByType")
                        .HasColumnType("integer");

                    b.Property<Guid?>("TriggeredByUserId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<int>("WorkflowType")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_workflow_events_tenant");

                    b.HasIndex("WorkflowType", "TargetEntityId", "TargetEntitySubKey")
                        .HasDatabaseName("ix_workflow_events_target");

                    b.ToTable("WorkflowEvents", "workflows");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.Workflows.WorkflowReview", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<bool>("Accepted")
                        .HasColumnType("boolean");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("DeclineReason")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<string>("DeletedBy")
                        .HasColumnType("text");

                    b.Property<string>("EditedContent")
                        .HasColumnType("text");

                    b.Property<Guid?>("ExternalParticipantInvitationId")
                        .HasColumnType("uuid");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<int>("ReviewerType")
                        .HasColumnType("integer");

                    b.Property<Guid?>("ReviewerUserId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("SubmittedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("TargetEntityId")
                        .HasColumnType("uuid");

                    b.Property<string>("TargetEntitySubKey")
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uuid");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UpdatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<int>("WorkflowType")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("ExternalParticipantInvitationId");

                    b.HasIndex("TenantId")
                        .HasDatabaseName("ix_workflow_reviews_tenant");

                    b.HasIndex("WorkflowType", "TargetEntityId", "TargetEntitySubKey")
                        .HasDatabaseName("ix_workflow_reviews_target");

                    b.ToTable("WorkflowReviews", "workflows");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<System.Guid>", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.Role", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<System.Guid>", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<System.Guid>", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<System.Guid>", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.Contact", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.Company", "Company")
                        .WithMany("Contacts")
                        .HasForeignKey("CompanyId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("QuantumBuild.Core.Domain.Entities.Site", "Site")
                        .WithMany()
                        .HasForeignKey("SiteId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Company");

                    b.Navigation("Site");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.DpaAcceptance", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.User", "AcceptedByUser")
                        .WithMany()
                        .HasForeignKey("AcceptedByUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Core.Domain.Entities.Tenant", "Tenant")
                        .WithMany()
                        .HasForeignKey("TenantId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("AcceptedByUser");

                    b.Navigation("Tenant");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.Employee", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.Department", "AssignedDepartment")
                        .WithMany()
                        .HasForeignKey("DepartmentId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("QuantumBuild.Core.Domain.Entities.Site", "PrimarySite")
                        .WithMany()
                        .HasForeignKey("PrimarySiteId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("AssignedDepartment");

                    b.Navigation("PrimarySite");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.LookupValue", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.LookupCategory", "Category")
                        .WithMany("Values")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Category");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.RolePermission", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.Permission", "Permission")
                        .WithMany("RolePermissions")
                        .HasForeignKey("PermissionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Core.Domain.Entities.Role", "Role")
                        .WithMany("RolePermissions")
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Permission");

                    b.Navigation("Role");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.Site", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.Company", "Company")
                        .WithMany("Sites")
                        .HasForeignKey("CompanyId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("QuantumBuild.Core.Domain.Entities.Employee", "SiteManager")
                        .WithMany()
                        .HasForeignKey("SiteManagerId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Company");

                    b.Navigation("SiteManager");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.SupervisorAssignment", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.Employee", "Operator")
                        .WithMany("OperatorAssignments")
                        .HasForeignKey("OperatorEmployeeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Core.Domain.Entities.Employee", "Supervisor")
                        .WithMany("SupervisorAssignments")
                        .HasForeignKey("SupervisorEmployeeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Operator");

                    b.Navigation("Supervisor");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.TenantLookupValue", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.LookupCategory", "Category")
                        .WithMany("TenantValues")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Core.Domain.Entities.LookupValue", "LookupValue")
                        .WithMany()
                        .HasForeignKey("LookupValueId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Category");

                    b.Navigation("LookupValue");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.User", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.Employee", "Employee")
                        .WithMany()
                        .HasForeignKey("EmployeeId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Employee");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.UserRole", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.Role", "Role")
                        .WithMany("UserRoles")
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Core.Domain.Entities.User", "User")
                        .WithMany("UserRoles")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Role");

                    b.Navigation("User");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.AuditCorpus", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.PipelineVersion", "FrozenFromPipelineVersion")
                        .WithMany()
                        .HasForeignKey("FrozenFromPipelineVersionId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "SourceTalk")
                        .WithMany()
                        .HasForeignKey("SourceTalkId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("FrozenFromPipelineVersion");

                    b.Navigation("SourceTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.AuditCorpusEntry", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.AuditCorpus", "Corpus")
                        .WithMany("Entries")
                        .HasForeignKey("CorpusId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Corpus");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ContentCreationSession", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourse", "OutputCourse")
                        .WithMany()
                        .HasForeignKey("OutputCourseId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_content_creation_sessions_course");

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "OutputTalk")
                        .WithMany()
                        .HasForeignKey("OutputTalkId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_content_creation_sessions_talk");

                    b.Navigation("OutputCourse");

                    b.Navigation("OutputTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.CorpusRun", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.AuditCorpus", "AuditCorpus")
                        .WithMany("Runs")
                        .HasForeignKey("CorpusId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.PipelineChangeRecord", "LinkedPipelineChange")
                        .WithMany()
                        .HasForeignKey("LinkedPipelineChangeId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.PipelineVersion", "PipelineVersion")
                        .WithMany()
                        .HasForeignKey("PipelineVersionId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("AuditCorpus");

                    b.Navigation("LinkedPipelineChange");

                    b.Navigation("PipelineVersion");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.CorpusRunResult", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.AuditCorpusEntry", "CorpusEntry")
                        .WithMany()
                        .HasForeignKey("CorpusEntryId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.CorpusRun", "CorpusRun")
                        .WithMany("Results")
                        .HasForeignKey("CorpusRunId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("CorpusEntry");

                    b.Navigation("CorpusRun");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.PipelineChangeRecord", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.PipelineVersion", "PipelineVersion")
                        .WithMany("ChangeRecords")
                        .HasForeignKey("PipelineVersionId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("PipelineVersion");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ProviderResultCache", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.AuditCorpusEntry", "CorpusEntry")
                        .WithMany()
                        .HasForeignKey("CorpusEntryId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("CorpusEntry");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.QrCode", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourse", "Course")
                        .WithMany()
                        .HasForeignKey("CourseId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.QrLocation", "QrLocation")
                        .WithMany("QrCodes")
                        .HasForeignKey("QrLocationId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "ToolboxTalk")
                        .WithMany()
                        .HasForeignKey("ToolboxTalkId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Course");

                    b.Navigation("QrLocation");

                    b.Navigation("ToolboxTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.QrSession", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.Employee", "Employee")
                        .WithMany()
                        .HasForeignKey("EmployeeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.QrCode", "QrCode")
                        .WithMany()
                        .HasForeignKey("QrCodeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Employee");

                    b.Navigation("QrCode");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryBody", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.Sector", "Sector")
                        .WithMany()
                        .HasForeignKey("SectorId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Sector");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryCriteria", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryProfile", "RegulatoryProfile")
                        .WithMany("Criteria")
                        .HasForeignKey("RegulatoryProfileId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Core.Domain.Entities.Tenant", null)
                        .WithMany()
                        .HasForeignKey("TenantId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("RegulatoryProfile");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryDocument", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryBody", "RegulatoryBody")
                        .WithMany("Documents")
                        .HasForeignKey("RegulatoryBodyId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("RegulatoryBody");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryProfile", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryDocument", "RegulatoryDocument")
                        .WithMany("Profiles")
                        .HasForeignKey("RegulatoryDocumentId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.Sector", "Sector")
                        .WithMany()
                        .HasForeignKey("SectorId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("RegulatoryDocument");

                    b.Navigation("Sector");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryRequirement", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryProfile", "RegulatoryProfile")
                        .WithMany()
                        .HasForeignKey("RegulatoryProfileId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("RegulatoryProfile");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryRequirementMapping", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourse", "Course")
                        .WithMany()
                        .HasForeignKey("CourseId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryRequirement", "RegulatoryRequirement")
                        .WithMany("Mappings")
                        .HasForeignKey("RegulatoryRequirementId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Core.Domain.Entities.Tenant", null)
                        .WithMany()
                        .HasForeignKey("TenantId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "ToolboxTalk")
                        .WithMany()
                        .HasForeignKey("ToolboxTalkId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Course");

                    b.Navigation("RegulatoryRequirement");

                    b.Navigation("ToolboxTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryStructureMap", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryDocument", "RegulatoryDocument")
                        .WithMany()
                        .HasForeignKey("RegulatoryDocumentId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("RegulatoryDocument");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryStructureMapFeature", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryStructureMapStandard", "RegulatoryStructureMapStandard")
                        .WithMany("Features")
                        .HasForeignKey("RegulatoryStructureMapStandardId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("RegulatoryStructureMapStandard");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryStructureMapPrinciple", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryStructureMap", "RegulatoryStructureMap")
                        .WithMany("Principles")
                        .HasForeignKey("RegulatoryStructureMapId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("RegulatoryStructureMap");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryStructureMapStandard", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryStructureMapPrinciple", "RegulatoryStructureMapPrinciple")
                        .WithMany("Standards")
                        .HasForeignKey("RegulatoryStructureMapPrincipleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("RegulatoryStructureMapPrinciple");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.SafetyGlossaryTerm", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.SafetyGlossary", "Glossary")
                        .WithMany("Terms")
                        .HasForeignKey("GlossaryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Glossary");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ScheduledTalk", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourseAssignment", "CourseAssignment")
                        .WithMany("ScheduledTalks")
                        .HasForeignKey("CourseAssignmentId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("QuantumBuild.Core.Domain.Entities.Employee", "Employee")
                        .WithMany()
                        .HasForeignKey("EmployeeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ScheduledTalk", "OriginalScheduledTalk")
                        .WithMany()
                        .HasForeignKey("OriginalScheduledTalkId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSchedule", "Schedule")
                        .WithMany()
                        .HasForeignKey("ScheduleId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "ToolboxTalk")
                        .WithMany()
                        .HasForeignKey("ToolboxTalkId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("CourseAssignment");

                    b.Navigation("Employee");

                    b.Navigation("OriginalScheduledTalk");

                    b.Navigation("Schedule");

                    b.Navigation("ToolboxTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ScheduledTalkCompletion", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ScheduledTalk", "ScheduledTalk")
                        .WithOne("Completion")
                        .HasForeignKey("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ScheduledTalkCompletion", "ScheduledTalkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ScheduledTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ScheduledTalkQuizAttempt", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ScheduledTalk", "ScheduledTalk")
                        .WithMany("QuizAttempts")
                        .HasForeignKey("ScheduledTalkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ScheduledTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ScheduledTalkSectionProgress", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ScheduledTalk", "ScheduledTalk")
                        .WithMany("SectionProgress")
                        .HasForeignKey("ScheduledTalkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSection", "Section")
                        .WithMany()
                        .HasForeignKey("SectionId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("ScheduledTalk");

                    b.Navigation("Section");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.SubtitleProcessingJob", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "ToolboxTalk")
                        .WithMany()
                        .HasForeignKey("ToolboxTalkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ToolboxTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.SubtitleTranslation", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.SubtitleProcessingJob", "ProcessingJob")
                        .WithMany("Translations")
                        .HasForeignKey("SubtitleProcessingJobId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ProcessingJob");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TenantReviewerConfiguration", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.Tenant", null)
                        .WithMany()
                        .HasForeignKey("TenantId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TenantSector", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.Sector", "Sector")
                        .WithMany("TenantSectors")
                        .HasForeignKey("SectorId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Core.Domain.Entities.Tenant", null)
                        .WithMany()
                        .HasForeignKey("TenantId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Sector");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TenantStandardSubscription", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryBody", "RegulatoryBody")
                        .WithMany()
                        .HasForeignKey("RegulatoryBodyId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Core.Domain.Entities.Tenant", null)
                        .WithMany()
                        .HasForeignKey("TenantId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("RegulatoryBody");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCertificate", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourse", "Course")
                        .WithMany()
                        .HasForeignKey("CourseId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("QuantumBuild.Core.Domain.Entities.Employee", "Employee")
                        .WithMany()
                        .HasForeignKey("EmployeeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "ToolboxTalk")
                        .WithMany()
                        .HasForeignKey("ToolboxTalkId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Course");

                    b.Navigation("Employee");

                    b.Navigation("ToolboxTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourseAssignment", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourse", "Course")
                        .WithMany()
                        .HasForeignKey("CourseId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Core.Domain.Entities.Employee", "Employee")
                        .WithMany()
                        .HasForeignKey("EmployeeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourseAssignment", "OriginalAssignment")
                        .WithMany()
                        .HasForeignKey("OriginalAssignmentId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Course");

                    b.Navigation("Employee");

                    b.Navigation("OriginalAssignment");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourseItem", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourse", "Course")
                        .WithMany("CourseItems")
                        .HasForeignKey("CourseId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "ToolboxTalk")
                        .WithMany()
                        .HasForeignKey("ToolboxTalkId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Course");

                    b.Navigation("ToolboxTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourseTranslation", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourse", "Course")
                        .WithMany("Translations")
                        .HasForeignKey("CourseId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Course");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkQuestion", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "ToolboxTalk")
                        .WithMany("Questions")
                        .HasForeignKey("ToolboxTalkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ToolboxTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSchedule", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "ToolboxTalk")
                        .WithMany()
                        .HasForeignKey("ToolboxTalkId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("ToolboxTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkScheduleAssignment", b =>
                {
                    b.HasOne("QuantumBuild.Core.Domain.Entities.Employee", "Employee")
                        .WithMany()
                        .HasForeignKey("EmployeeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSchedule", "Schedule")
                        .WithMany("Assignments")
                        .HasForeignKey("ScheduleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Employee");

                    b.Navigation("Schedule");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSection", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "ToolboxTalk")
                        .WithMany("Sections")
                        .HasForeignKey("ToolboxTalkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ToolboxTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSlide", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "ToolboxTalk")
                        .WithMany("Slides")
                        .HasForeignKey("ToolboxTalkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ToolboxTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSlideTranslation", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSlide", "Slide")
                        .WithMany("Translations")
                        .HasForeignKey("SlideId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Slide");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSlideshowTranslation", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "ToolboxTalk")
                        .WithMany("SlideshowTranslations")
                        .HasForeignKey("ToolboxTalkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ToolboxTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkTranslation", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "ToolboxTalk")
                        .WithMany("Translations")
                        .HasForeignKey("ToolboxTalkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ToolboxTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkVideoTranslation", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "ToolboxTalk")
                        .WithMany("VideoTranslations")
                        .HasForeignKey("ToolboxTalkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ToolboxTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TranslationDeviation", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TranslationValidationRun", "ValidationRun")
                        .WithMany()
                        .HasForeignKey("ValidationRunId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("ValidationRun");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TranslationFlag", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "ToolboxTalk")
                        .WithMany()
                        .HasForeignKey("ToolboxTalkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TranslationValidationResult", "ValidationResult")
                        .WithMany("Flags")
                        .HasForeignKey("ValidationResultId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ToolboxTalk");

                    b.Navigation("ValidationResult");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TranslationValidationResult", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TranslationValidationRun", "ValidationRun")
                        .WithMany("Results")
                        .HasForeignKey("ValidationRunId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ValidationRun");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TranslationValidationRun", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourse", "Course")
                        .WithMany()
                        .HasForeignKey("CourseId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.PipelineVersion", "PipelineVersion")
                        .WithMany("Runs")
                        .HasForeignKey("PipelineVersionId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", "ToolboxTalk")
                        .WithMany()
                        .HasForeignKey("ToolboxTalkId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Course");

                    b.Navigation("PipelineVersion");

                    b.Navigation("ToolboxTalk");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ValidationRegulatoryScore", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryProfile", "RegulatoryProfile")
                        .WithMany()
                        .HasForeignKey("RegulatoryProfileId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TranslationValidationRun", "ValidationRun")
                        .WithMany()
                        .HasForeignKey("ValidationRunId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("RegulatoryProfile");

                    b.Navigation("ValidationRun");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.Workflows.WorkflowReview", b =>
                {
                    b.HasOne("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.Workflows.ExternalParticipantInvitation", "ExternalParticipantInvitation")
                        .WithMany()
                        .HasForeignKey("ExternalParticipantInvitationId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("ExternalParticipantInvitation");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.Company", b =>
                {
                    b.Navigation("Contacts");

                    b.Navigation("Sites");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.Employee", b =>
                {
                    b.Navigation("OperatorAssignments");

                    b.Navigation("SupervisorAssignments");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.LookupCategory", b =>
                {
                    b.Navigation("TenantValues");

                    b.Navigation("Values");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.Permission", b =>
                {
                    b.Navigation("RolePermissions");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.Role", b =>
                {
                    b.Navigation("RolePermissions");

                    b.Navigation("UserRoles");
                });

            modelBuilder.Entity("QuantumBuild.Core.Domain.Entities.User", b =>
                {
                    b.Navigation("UserRoles");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.AuditCorpus", b =>
                {
                    b.Navigation("Entries");

                    b.Navigation("Runs");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.CorpusRun", b =>
                {
                    b.Navigation("Results");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.PipelineVersion", b =>
                {
                    b.Navigation("ChangeRecords");

                    b.Navigation("Runs");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.QrLocation", b =>
                {
                    b.Navigation("QrCodes");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryBody", b =>
                {
                    b.Navigation("Documents");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryDocument", b =>
                {
                    b.Navigation("Profiles");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryProfile", b =>
                {
                    b.Navigation("Criteria");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryRequirement", b =>
                {
                    b.Navigation("Mappings");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryStructureMap", b =>
                {
                    b.Navigation("Principles");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryStructureMapPrinciple", b =>
                {
                    b.Navigation("Standards");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.RegulatoryStructureMapStandard", b =>
                {
                    b.Navigation("Features");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.SafetyGlossary", b =>
                {
                    b.Navigation("Terms");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ScheduledTalk", b =>
                {
                    b.Navigation("Completion");

                    b.Navigation("QuizAttempts");

                    b.Navigation("SectionProgress");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.Sector", b =>
                {
                    b.Navigation("TenantSectors");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.SubtitleProcessingJob", b =>
                {
                    b.Navigation("Translations");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalk", b =>
                {
                    b.Navigation("Questions");

                    b.Navigation("Sections");

                    b.Navigation("Slides");

                    b.Navigation("SlideshowTranslations");

                    b.Navigation("Translations");

                    b.Navigation("VideoTranslations");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourse", b =>
                {
                    b.Navigation("CourseItems");

                    b.Navigation("Translations");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkCourseAssignment", b =>
                {
                    b.Navigation("ScheduledTalks");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSchedule", b =>
                {
                    b.Navigation("Assignments");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.ToolboxTalkSlide", b =>
                {
                    b.Navigation("Translations");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TranslationValidationResult", b =>
                {
                    b.Navigation("Flags");
                });

            modelBuilder.Entity("QuantumBuild.Modules.ToolboxTalks.Domain.Entities.TranslationValidationRun", b =>
                {
                    b.Navigation("Results");
                });
#pragma warning restore 612, 618
        }
    }
}

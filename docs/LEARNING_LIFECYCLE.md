...
A migration's filename describes the _full schema effect_ of the
...
**What:** When recon for a chunk surfaces what existing code _can_
...
_also_ read the `NeedsRevalidation` flag and `MarkedStale` workflow
...
the first question is _what does this code currently exist to do?_
...
### 8.4 Design-doc forward-reference may hide additive

requirements in a downstream section

**Pattern:** A canonical specification section (e.g., §7.1 "New
Wizard" in TRANSLATION_WORKFLOW_DESIGN.md) defines a UI surface
at one level of detail. A later section (e.g., §9's per-phase
build-plan description) adds requirements not present in the
canonical section. A recon that reads only the canonical
section, or treats it as the spec of record, will miss these
additive requirements and either implement against the thinner
spec (regressing UX vs. the build plan) or rediscover the
additions late, during implementation or review.

**How it surfaced:** Phase 3c (2026-06-09). §7.2 "Edit Page"
forward-references §7.1 with "Same per-language control panel
as the wizard. Operations are the same." §7.1 lists "current
state" as the panel's rendered field — that's it. §9's Phase
3c description adds: "Each language renders: current workflow
state, last validation outcome, per-language actions." The
build-plan description thus quietly committed the panel to
showing last validation outcome, which §7.1 does not mention.
An initial recon reading only §7.1 would have missed this and
needed a second recon pass.

**Why it happens:** Design docs evolve. The canonical UI
section gets written first, capturing the intent at the time.
The build plan gets written later, capturing what each phase
actually has to build — which is often more than the canonical
section captured. Updating the canonical section to match the
build plan is the right discipline but easy to skip. The
build-plan description becomes the de facto spec without
being reflagged as such.

**Mitigation:**

- Recon for any phase implementing a section of the design doc
  must read BOTH the canonical section AND the corresponding
  build-plan description in §9. Treat them as a pair, not as
  primary + supplementary.
- When the build-plan description is more specific than the
  canonical section, the discrepancy is a doc bug. Resolve by
  amending the canonical section before implementing, so the
  doc on disk is the doc that ships. Phase 3c did exactly
  this — the §7.1 amendments in commit 38ba9c8 and e04b607
  recorded decisions that should have been in §7.1 all along.
- When writing a new build-plan description, check whether
  you're committing to anything the canonical section doesn't
  cover. If so, update the canonical section in the same
  commit. Don't let the build plan be the only place a
  requirement lives.

**Surfaced:** 2026-06-09, during Phase 3c initial recon and
the subsequent doc-recon pass.

**Related:** §8.12 (scope-drift via capability-listing during
recon) is the inverted shape — recon adds capabilities the
design doc didn't ask for. §8.13 is the design doc adding
capabilities the canonical section didn't ask for. Both
result in the same symptom (mismatch between what gets built
and what was thought to be in scope) from opposite directions.
...
_Document last revised: 2026-06-05. §1–§9 verified against commit `6bf1a71` (HEAD of `transval` branch at time of §6 batch). §10 verified against HEAD `bb2709e`. Re-verify file paths and line numbers before relying on them after any significant refactor._

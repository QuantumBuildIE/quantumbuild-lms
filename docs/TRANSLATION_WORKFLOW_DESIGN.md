...
Public-facing page for token-scoped external reviews. Token issuance and verification. Email template. Submit/cancel endpoints. Generic enough to be reused by future workflow types.
  ...
  **Status (Phase 4.5a):** Complete (2026-06-09). Commit 52ec27e. Closes a foundational gap identified by recon at commit 33f4f4b: `WorkflowReview.EditedContent` from external submissions was stored but orphaned — `ConfirmExternalReview` wrote a WorkflowEvent and returned `Ok` without ever reading the reviewer's edits, meaning the canonical `ToolboxTalkTranslation.TranslatedSections` (the field employees actually read for translated content) never reflected the reviewer's work. Phase 4.5a adds 
  `PropagateExternalReviewEditsAsync` to `TranslationWorkflowService`: on `ConfirmExternalReview(accepted=true)`, the helper loads the most recent accepted external `WorkflowReview` for the (talkId, languageCode) pair, deserialises `EditedContent` as `List<ExternalReviewEditedSectionDto>` (JSON array of `[{ sectionIndex, translatedText }]`), loads the `ToolboxTalkTranslation` row, deserialises `TranslatedSections` via a private `TranslatedSectionEntry` class preserving 
  `SectionId`/`Title`/`Content` shape, applies each edit to the corresponding entry's `Content`, re-serialises, and lets the caller's existing `SaveChangesAsync` persist alongside the `WorkflowEvent`. Defensive logging covers null/malformed JSON, missing translation rows, out-of-range section indices. The `accepted=false` path explicitly does NOT propagate — rejected edits remain advisory on the review row but don't go live. New shared DTO `ExternalReviewEditedSectionDto` defines the wire contract that Phase 4.5b's portal will produce. XML comment on `WorkflowReview.EditedContent` documents the dual semantics — internal flow stores per-section content, external flow stores the JSON array. Five new integration tests cover accepted-true propagation, accepted-false non-propagation, partial edits (only edited sections change, others preserved), malformed JSON (logged and skipped, confirm flow continues), and out-of-range section indices (skipped with warning). 398/398 tests passing, no new warnings. Phase 4.5b (portal page) consumes this contract; 4.6 is the panel UI for Send/Cancel.

  **Status (Phase 4.5b):** Complete (2026-06-09). Commit 9d81f2a. Public portal page at `web/src/app/external-review/[token]/page.tsx`, 
  outside the `(authenticated)` route group so the root layout wraps it without auth redirect. Follows the QR scan page precedent for public-page patterns: `"use client"`, `useParams` for the token segment, raw `fetch` against `NEXT_PUBLIC_API_URL` (no `apiClient`, which would attach a 
  Bearer interceptor). Page-level state machine has nine `Step` values — `loading`, `active`, `used`, `revoked`, `expired`, `not-found`, `submitted`, `declined`, `error` — mapped from the backend response status and `PortalStatus` field. The Active state renders a header card (talk title, language, expiry) plus one card per section with source on the left (rendered via reused `FlaggedText` from the validation work, through a small adapter from `ExternalReviewFlagDto` to the expected `TranslationFlag` shape) and an editable Textarea for the translation on the right. Layout is 
  `grid grid-cols-1 md:grid-cols-2 gap-4` — stacked on mobile, side-by-side at 768px+. Action buttons (Decline outlined, Submit primary) at the bottom of the page; both stack on mobile via `flex flex-col sm:flex-row gap-3`. The Submit confirmation is an inlined Dialog (single use site, no 
  abstraction needed). The Decline modal is its own component at `web/src/features/external-review/components/DeclineConfirmationDialog.tsx` 
  with a mandatory non-empty textarea (Submit button disabled while `reason.trim() === ""`) and a useEffect that resets the reason on open. Submit handler serialises `editedSections` state (a `Record<number, string>` keyed by sectionIndex) into the Phase 4.5a JSON contract `[{ sectionIndex, translatedText }, ...]` and POSTs to `/api/external-review/{token}/submit`. Decline handler POSTs the reason. Successful submission transitions to the `submitted` step (thank-you card replaces the editing UI); successful decline transitions to `declined`. Network errors surface via Sonner toast; state-changing errors transition to the relevant terminal state. New TS types in `web/src/types/external-review.ts` mirror the Phase 4.3/4.5a backend DTOs. New API wrappers in `web/src/lib/api/external-review.ts` are pure `fetch` functions returning `{ status, body }` pairs that callers switch on. TypeScript build passes with zero errors and zero warnings introduced. With 4.5b shipped, the external participant flow is functional end-to-end: requester sends → email reaches reviewer → reviewer edits in portal → submits → internal team confirms → edits propagate to 
  `TranslatedSections` that employees read. The remaining Phase 4 work (4.6) is the panel-side UI for Send/Cancel, which exposes the existing backend endpoints to the requester from the admin UI.

  **Status (Phase 4.6):** Complete (2026-06-09). Commit 1bf75f6. Closes Phase 4. Adds Send for External Review and Cancel External Review buttons to the per-language `TranslationWorkflowPanel`. Send button renders only when state is `ReviewerAccepted`; Cancel only when state is `AwaitingThirdParty`. Send opens a confirmation dialog showing the flagged word count for the language (sourced from a new `FlaggedWordCount` field on `TranslationWorkflowStateDto`, populated in `GetState` via the existing `ComputeFlaggedWordCountAsync` helper — adds one query per state lookup, acceptable per the design call) and an email input; on confirm fires `POST /api/toolbox-talks/{id}/translations/{lang}/initiate-external-review` 
  via a new controller action `InitiateTranslationExternalReview` (the service method already existed from earlier work; the endpoint was missing). Cancel opens a destructive-variant confirmation dialog and fires the Phase 4.1 cancel endpoint. Both mutations invalidate the workflow-state and workflow-history query keys on success; success surfaces as a toast and the panel re-renders with the new state. New components  `SendExternalReviewDialog` and `CancelExternalReviewDialog` under `web/src/features/toolbox-talks/components/`, both matching the shadcn/ui Dialog pattern established by `DeclineConfirmationDialog` in Phase 4.5b. API wrappers and React Query mutation hooks added to the existing toolbox-talks 
  client. New `InitiateExternalReviewRequest` record at the end of `ToolboxTalksController.cs` matches the JSON body shape `{ reviewerEmail }`. Two new backend integration tests assert `FlaggedWordCount` populates correctly (positive case with three flags totalling four words, zero case with no validation run); 402/402 tests passing. TypeScript build clean.

  **Phase 4 closure (2026-06-09):** All six sub-chunks shipped. The external participant flow is end-to-end functional: requester sends invitation from the per-language panel → reviewer receives an email with portal link → reviewer edits the translation in the portal and submits → invitation marked Used, edits stored on `WorkflowReview.EditedContent` → workflow transitions to `ThirdPartyReviewed` → internal team confirms via existing flow → propagation copies edits into `ToolboxTalkTranslation.TranslatedSections` → employees see the reviewed translation. Eight Phase 4 commits across backend and frontend (4.1 / 4.2a / 4.2b / 4.3 / 4.4 / 4.5a / 4.5b / 4.6). Known follow-up work: Phase 7 will add the general notification trigger (currently several TODO comments at the points where Phase 4 emits workflow events); BACKLOG §5.6 covers MailerSend resilience improvements; BACKLOG §11 (Cancel external review end-to-end) is now closed by 4.1 + 4.6.

...
| 4 | `TranslatingValidating` | 4 | Translation Running | One or more `TranslationValidationJob` Hangfire jobs are in flight. Session holds the run IDs in `ValidationRunIds`. | **In-flight** |
...
- **Also:** `ConfirmUploadAsync` (presigned upload path, [service:1258](src/Modules/ToolboxTalks/QuantumBuild.Modules.ToolboxTalks.Infrastructure/Services/ContentCreation/ContentCreationSessionService.cs#L1258)) — also resets to Draft. **No status guard** — resets from any status, including Parsed/Validated (see §6.11).
...
- **Trigger:** `GenerateQuizAsync` ([service:933](src/Modules/ToolboxTalks/QuantumBuild.Modules.ToolboxTalks.Infrastructure/Services/ContentCreation/ContentCreationSessionService.cs#L933)) — called from `POST /api/toolbox-talks/create/session/{id}/generate-quiz`.
- **Also valid from:** `Validated`, `QuizGenerated`, `GeneratingQuiz` (re-generate).
- **Data written:** `Status = GeneratingQuiz` saved immediately (line 954–955). AI calls happen synchronously during the HTTP request. On success: `QuestionsJson`, `QuizSettingsJson` initialized if empty, `Status = QuizGenerated` — single `SaveChangesAsync` (line 1023).
- **Atomic?** No — two separate saves (status transition, then result).
- **Failure mode:** Exception catch sets `Status = Failed` (line 1036). If that save fails, session stays `GeneratingQuiz`.

---

#### Parsed/QuizGenerated/Validated → TranslatingValidating (or Validated)
- **Trigger:** `StartTranslateValidateAsync` ([service:384](src/Modules/ToolboxTalks/QuantumBuild.Modules.ToolboxTalks.Infrastructure/Services/ContentCreation/ContentCreationSessionService.cs#L384)) — called from `POST /api/toolbox-talks/create/session/{id}/translate-validate`.
...
- **Data written — large multi-step operation:**
  ...
  | Step | Lines | Action | Table/Field |
  |------|-------|--------|------------|
  | 1 | 432–571 | Create OR reuse draft `ToolboxTalk` | `ToolboxTalks` row created or existing metadata updated |
  | 2 | 458–460 | Soft-delete existing `ToolboxTalkSections` | `ToolboxTalkSections.IsDeleted = true` |
  | 3 | 463–474 | Insert new `ToolboxTalkSections` | New rows from `ParsedSectionsJson` |
  | 4 | 477–481 | **Hard-delete** existing `ToolboxTalkQuestions` | `Remove()` — gone from DB on `SaveChanges` |
  | 5 | 483–485 | Insert new `ToolboxTalkQuestions` | From `QuestionsJson` (skipped when `IncludeQuiz == false`) |
  | 6 | 492–497 | **Hard-delete** all `ToolboxTalkTranslations` | `IgnoreQueryFilters()` + `Remove()` — including soft-deleted rows |
  | 7 | 573–586 | Set `VideoUrl` / `VideoSource` on draft talk if video | `ToolboxTalks` columns |
  | 8 | 618 | Update `session.TargetLanguageCodes` | Overwritten from request, not from original session creation |
  | 9 | 619 | `Status = TranslatingValidating` (or `Validated` if no targets) | `ContentCreationSessions.Status` |
  | 10 | 625–645 | Create one `TranslationValidationRun` per language | New rows, `Status = Pending` |
  | 11 | 647 | Serialize run IDs to `session.ValidationRunIds` | JSON array of `Guid` |
  | 12 | 650–658 | `SaveChangesAsync` — **all of the above in one save** | All tables above |
  | 13 | 661–665 | Enqueue `TranslationValidationJob` per run | Hangfire, **after** save |
  | 14 | 668–706 | (Video only) Start subtitle processing | `SubtitleProcessingJob` + updates `session.SubtitleJobId` (separate save at line 694) |

- **Fast path (no targets):** Steps 1–8 still execute, but step 9 writes `Validated`, steps 10–13 are skipped. `ValidationRunIds` is **not set** (stays null or retains previous value). Steps 1–9 in one save.
- **Atomic?** Steps 1–12 are atomic (single `SaveChangesAsync`). Step 13 (enqueue) happens after the save and is not transactional. Step 14 is a separate save.
- **Failure mode:** If save fails, no jobs are enqueued. Session stays at its prior status. If save succeeds but Hangfire enqueue fails (line 663), runs are in `Pending` with no job executing them — permanently stuck until expiry. This is the **orphan-pending-run** scenario. If subtitle processing (line 685) throws, the exception is caught and logged; main flow continues.

---

#### TranslatingValidating → Validated (TranslationValidationJob)
- **Trigger:** `TryUpdateSessionStatusAsync` ([TranslationValidationJob.cs:937](src/Modules/ToolboxTalks/QuantumBuild.Modules.ToolboxTalks.Infrastructure/Jobs/TranslationValidationJob.cs#L937)) — called at the end of each `TranslationValidationJob.ExecuteAsync` (line 255).
...
- **Data written:** `Status = Validated` (line 983), `SaveChangesAsync` (line 984).
- **Atomic?** Yes.
- **Failure mode:** The entire `TryUpdateSessionStatusAsync` is wrapped in a try/catch that only logs a warning (line 994). If it throws, the session stays `TranslatingValidating` without any error surfaced. The next job completing for the same session (if multiple languages) will retry the check.
...
- **Data written:**
  ...
  2. `PublishAsLessonAsync` or `PublishAsCourseAsync` (large operations — see §4.8).
...
- **Atomic?** No — `Status = Publishing` is committed before the publish logic runs. The publish logic does one large `SaveChangesAsync` at its end (line 1588 for lesson, line 2059 for course), but accumulates many EF changes beforehand. If anything throws, the session is set to `Failed` via catch blocks.
...
- **Trigger:** `UpdateSectionsAsync` ([service:319](src/Modules/ToolboxTalks/QuantumBuild.Modules.ToolboxTalks.Infrastructure/Services/ContentCreation/ContentCreationSessionService.cs#L319)) — called from `PUT /api/toolbox-talks/create/session/{id}/sections`.
- **In-flight guard (lines 327–345):** If status is in `{Transcribing, Parsing, GeneratingQuiz, TranslatingValidating, Publishing}`, throws `InvalidOperationException` with message "Section editing is not available while {status} is in progress." Returns HTTP 400. The frontend **can** navigate to ParseStep (step 2) in these states (see §7 Q2 — resolved), but any save attempt is rejected.
- **Allowed statuses:** `{Parsed, QuizGenerated, Validated}`.
- **Data written (single SaveChangesAsync, line 375):**
...
- **Atomic?** Yes — single `SaveChangesAsync` (line 375).
...
#### QuizGenerated/Validated/Parsed → QuizGenerated (UpdateQuestionsAsync)
- **Trigger:** `UpdateQuestionsAsync` ([service:1071](src/Modules/ToolboxTalks/QuantumBuild.Modules.ToolboxTalks.Infrastructure/Services/ContentCreation/ContentCreationSessionService.cs#L1071)) — called from `PUT /api/toolbox-talks/create/session/{id}/questions`.
- **Allowed statuses (line 1079–1082):** `{QuizGenerated, Validated, Parsed}`. Other statuses throw.
- **Data written (single SaveChangesAsync, line 1090):**
  - `QuestionsJson` — overwritten.
  - `Status` — set to `QuizGenerated` if current status is **not** already `QuizGenerated` (line 1087–1088). This means editing questions from `Validated` **silently demotes the session back to `QuizGenerated`** and `StartTranslateValidateAsync` must be re-run to re-sync changes (see §6.10).
- **Data NOT cleared:** `ValidationRunIds`, `SettingsJson`, `ParsedSectionsJson`. Old translations on the draft talk are not touched.
- **Atomic?** Yes — single `SaveChangesAsync` (line 1090).

---

#### Any → Any (UpdateQuizSettingsAsync)
- **Trigger:** `UpdateQuizSettingsAsync` ([service:1099](src/Modules/ToolboxTalks/QuantumBuild.Modules.ToolboxTalks.Infrastructure/Services/ContentCreation/ContentCreationSessionService.cs#L1099)) — called from `PUT /api/toolbox-talks/create/session/{id}/quiz-settings`.
- **No status guard** — allowed from any status.
- **Data written (single SaveChangesAsync, line 1108):** `QuizSettingsJson` only. No status change, no cascade.
...
- **No status guard** — allowed from any status.
- **Data written (single SaveChangesAsync, line 1155):** `SettingsJson` only. No status change, no cascade.
...
- **Data written (single SaveChangesAsync, line 172):**
  ...
  - `OutputType = null`.
- **Data NOT cleared:** `QuestionsJson`, `ValidationRunIds`, `SettingsJson`, `SubtitleJobId`, `TargetLanguageCodes`, `TranscriptText`. After this call the session is `Draft` with downstream fields in a potentially inconsistent state until re-parsed.
...
Draft ──parse──→ Transcribing ──job──→ Parsing ──job──→ Parsed ←──────────────────┐
  ...
  └──parse (pdf/text)───────────────────────────────────────┘  └── QuizGenerated  │
                                     ...
                                     Validated ←──────────────────────────────────┘
                                        │   ↑
                                        │   └── [update-questions from Validated → demotes to QuizGenerated]
                                        │
                                    [publish]
                                        │
                                     Publishing
...
| `TargetLanguageCodes` | Session create **and** overwritten in StartTranslateValidateAsync (line 618) | TranslateStep, ValidateStep (language tabs) |
...
| `ValidationRunIds` | StartTranslateValidateAsync (lines 647, 596–597) | TranslateStep (fetches run details), ValidateStep (reviewer decisions), TryUpdateSessionStatusAsync (run completion check), Frontend `isStepReachable` guard (steps 5+6 blocked when null/empty). **null = no validation was needed OR cascade reset cleared it** |
...
| `SubtitleJobId` | StartTranslateValidateAsync (video only, line 693) | Not read by service after write — informational |
...
| **Promoted to Published** | `PublishAsLessonAsync` — sets `Status = Published`, all behaviour fields, video/PDF URLs. Full trace in §4.8. |
| **Repurposed as Full Video** | `PublishAsCourseAsync` (Video mode) — draft talk is renamed to "{title} — Full Video", sections replaced with a single placeholder section, quiz cleared. Full trace in §4.8. |
...
| **Soft-deleted on re-run** | `StartTranslateValidateAsync` (lines 458–460) — old sections soft-deleted before new ones inserted. | `IsDeleted = true` |
| **Soft-deleted on publish** | `PublishAsLessonAsync` (lines 1395–1396) — old sections soft-deleted, new ones inserted. | `IsDeleted = true` |
...
| **Hard-deleted on re-run** | `StartTranslateValidateAsync` (lines 477–481) — `Remove()` called, deleted on `SaveChanges`. | **Hard delete** |
| **Hard-deleted on publish** | `PublishAsLessonAsync` (lines 1431–1435) — same pattern. | **Hard delete** |
| **No query filter** | Unlike sections, questions are hard-deleted; no accumulation. |
...
| **Hard-deleted on re-run** | `StartTranslateValidateAsync` (lines 492–497) — uses `IgnoreQueryFilters()` to catch ALL rows including previously soft-deleted ones, then `Remove()`. | **Hard delete** |
| **Remapped on lesson publish** | `PublishAsLessonAsync` (lines 1458–1538) — `TranslatedSections` and `TranslatedQuestions` JSON are rewritten to replace old draft section/question IDs with new published IDs. Full trace in §4.8. |
...
| **Critical invariant** | The hard-delete in `StartTranslateValidateAsync` is the only mechanism preventing the unique-constraint violation on re-run. If this code is bypassed or executed out of order, the job fails with `23505 duplicate key value`. |

### 4.5 TranslationValidationRun

| Event | Operation |
|-------|-----------|
| **Created** | `StartTranslateValidateAsync` (lines 625–644) — one per filtered target language, `Status = Pending`. |
| **Updated** | `TranslationValidationJob` — status updates (`Running → Completed/Failed`), aggregate scores, `SafetyVerdict`. |
| **Orphaned on cascade reset** | When `UpdateSectionsAsync` runs from `Validated` status, `session.ValidationRunIds` is set to null but the `TranslationValidationRun` rows are NOT deleted. They remain in the DB linked to the draft talk. New runs created by the next `StartTranslateValidateAsync` call are separate rows; old runs become orphaned. Orphaned runs remain accessible via the talk's validation history endpoints. |
| **Reassociated on course publish** | `PublishAsCourseAsync` (lines 2062–2081) — `run.CourseId = course.Id`, `run.ToolboxTalkId = null`. Separate `SaveChangesAsync` at line 2076. |
...
### 4.6 TranslationValidationResult
...
### 4.8 Publish Path — Full Trace

#### `PublishAsLessonAsync` — Draft Talk Promote Path (lines 1313–1596)

This path executes when `session.OutputTalkId != null` (a draft talk was created by `StartTranslateValidateAsync`). All changes are accumulated in-memory and flushed in a **single `SaveChangesAsync` at line 1588**.

| Step | Lines | Action |
|------|-------|--------|
| 1 | 1325–1328 | Load draft talk (`IgnoreQueryFilters`, `!IsDeleted`) |
| 2 | 1332–1335 | Validate title uniqueness (excluding the draft talk itself) |
| 3 | 1338–1340 | Capture `oldTitle` / `oldDescription` for staleness detection |
| 4 | 1341–1349 | Set `Title`, `Description`, `Category`, `SourceLanguageCode`, `Status = Published`, `VideoUrl/VideoSource` or `PdfUrl/PdfFileName` |
| 5 | 1352–1376 | Apply behaviour fields from `SettingsJson`: `IsActive`, `GenerateCertificate`, `MinimumVideoWatchPercent`, `AutoAssignToNewEmployees`, `AutoAssignDueDays`, `RequiresRefresher`, `RefresherIntervalMonths` |
| 6 | 1379–1386 | Re-apply custom `Code` from request if provided (validated for uniqueness) |
| 7 | 1390–1394 | Load existing section rows; capture ordered `oldSectionIds` |
| 8 | 1395–1396 | Soft-delete old sections (`IsDeleted = true`) |
| 9 | 1398–1412 | Insert new sections from `ParsedSectionsJson`; collect `newSectionIds` in same order |
| 10 | 1417–1449 | (IncludeQuiz only) Capture `oldQuestionIds`; hard-delete old questions; insert new questions from `QuestionsJson`; collect `newQuestionIds` from change tracker |
| 11 | 1458–1490 | **Remap `TranslatedSections`**: for each `ToolboxTalkTranslation` (via `IgnoreQueryFilters`), iterate old→new section ID pairs; call `ExtractTranslatedSectionForId` to extract + remap one element; write new JSON array |
| 12 | 1493–1538 | **Remap `TranslatedQuestions`**: same pattern for question IDs. Skipped with warning log if question counts don't match. |
| 13 | 1541–1586 | If `oldTitle != request.Title` OR `oldDescription != request.Description`: re-translate for each language via `_contentTranslationService.TranslateTextAsync`; update `TranslatedTitle`/`TranslatedDescription` on each translation row |
| 14 | 1588 | **Single `SaveChangesAsync`** — commits steps 4–13 atomically |
| 15 | 1591–1592 | Enqueue `RequirementMappingJob` (fire-and-forget) |

**`ExtractTranslatedSectionForId`** ([service:2143](src/Modules/ToolboxTalks/QuantumBuild.Modules.ToolboxTalks.Infrastructure/Services/ContentCreation/ContentCreationSessionService.cs#L2143)) and **`ExtractTranslatedQuestionForId`** ([service:2104](src/Modules/ToolboxTalks/QuantumBuild.Modules.ToolboxTalks.Infrastructure/Services/ContentCreation/ContentCreationSessionService.cs#L2104)): static helpers that parse the JSON array, find the element by old ID, replace with new ID, and return a single-element JSON array. If the JSON is malformed or the ID is not found, return `null` — the affected translation entry is silently omitted from the remapped array.

#### `PublishAsLessonAsync` — No-Draft Fallback Path (lines 1598–1699)
...
#### `PublishAsCourseAsync` — Course Distribution Path (lines 1701–2096)

Draft translations and sections are loaded **before** any mutations at lines 1731–1766 to prevent data loss during distribution.

| Step | Lines | Action |
|------|-------|--------|
| 1 | 1724–1729 | Validate course title uniqueness |
| 2 | 1731–1766 | Capture draft sections, draft translations, draft question IDs BEFORE mutations |
| 3 | 1768–1776 | Create `ToolboxTalkCourse` entity |
| 4a | 1782–1853 | **(Video mode only)** Repurpose draft talk as "Full Video" course item (OrderIndex 0): rename, soft-delete draft sections, insert placeholder "Video" section, hard-delete questions, update translation rows (title suffix, `TranslatedSections = "[]"`, `TranslatedQuestions = null`), regenerate Code |
| 4b | 1857–1996 | For each section: create a new Published `ToolboxTalk` with that section as its sole section. Copy full quiz question set. Migrate per-section translation from draft translations (extract+remap section+question IDs). Create `ToolboxTalkTranslation` rows per language. Add as course item. |
| 5 | 1998–2056 | Create `ToolboxTalkCourseTranslation` rows from draft translations. Re-translate title/description if changed at publish time. |
| 6 | 2058–2059 | `SaveWithCodeRetryAsync` — commits steps 3–5 atomically (up to 10 code retry attempts) |
| 7 | 2062–2081 | Reassociate validation runs: `run.CourseId = course.Id`, `run.ToolboxTalkId = null`. Second `SaveChangesAsync` at line 2076 (only if runs > 0). |
| 8 | 2085–2089 | **(Non-Video only)** `DeleteDraftTalkAsync`: soft-delete draft talk + sections + translations; hard-delete questions; single `SaveChangesAsync` at line 2227. Set `session.OutputTalkId = null`. |
| 9 | 2092–2093 | Enqueue `RequirementMappingJob` (fire-and-forget) |
...
| `TranslationValidationJob` | `content-generation` | 1 | `ContentCreationSessionService.StartTranslateValidateAsync` | [service:663](src/Modules/ToolboxTalks/QuantumBuild.Modules.ToolboxTalks.Infrastructure/Services/ContentCreation/ContentCreationSessionService.cs#L663) | One per target language |
...
| `TranslationValidationJob` | Run → `Completed`; calls `TryUpdateSessionStatusAsync`; if all runs done, session → `Validated` | Run → `Failed`; session may stay `TranslatingValidating` | Run status is whatever it was before the exception; session stays `TranslatingValidating` indefinitely |
...
The transition from `TranslatingValidating` to `Validated` happens inside `TryUpdateSessionStatusAsync` ([TranslationValidationJob.cs:937](src/Modules/ToolboxTalks/QuantumBuild.Modules.ToolboxTalks.Infrastructure/Jobs/TranslationValidationJob.cs#L937)):

1. Load all sessions for the tenant with `Status == TranslatingValidating` and `ValidationRunIds != null` (lines 942–948).
2. Build `runIdString = validationRunId.ToString()` (line 950).
3. For each session, substring-check `session.ValidationRunIds.Contains(runIdString)` (line 954) — fast pre-filter.
4. Deserialize `ValidationRunIds` JSON to `List<Guid>` (line 961).
5. Check `runIds.Contains(validationRunId)` (line 968) — precise match after deserialization.
6. Load statuses for all run IDs (filter: `!r.IsDeleted`) — lines 972–976.
7. `allComplete = allRunStatuses.Count > 0 && allRunStatuses.All(s == Completed || s == Failed)` (lines 978–979).
8. If `allComplete`, set `Status = Validated` (line 983), `SaveChangesAsync` (line 984).

**Edge cases:**
- If a run row is soft-deleted between step 6 and step 7 (unlikely), `allComplete` could be true with fewer runs than expected.
- If `ValidationRunIds` JSON is malformed, `JsonSerializer.Deserialize` throws at line 961, the inner `catch` at line 963 silently `continue`s, and the session never transitions.
- `TryUpdateSessionStatusAsync` is wrapped in its own outer try/catch (lines 939/994) — any exception is swallowed as a warning.
...
### 6.2 Unfiltered Unique Index on `ToolboxTalkTranslation`
...
**Current mitigation:** `StartTranslateValidateAsync` (lines 492–497) hard-deletes ALL translations for the talk using `IgnoreQueryFilters()` before creating new ones. This runs on every re-run regardless of cascade state.

**Why it can still bite:** The hard-delete runs only in `StartTranslateValidateAsync`. If a translation row is somehow inserted between that call and the job's own insert (e.g., `DailyTranslationScanJob` fires a `MissingTranslationsJob` for the same talk concurrently), the job's insert can still collide.
...
### 6.4 Orphaned Hangfire Jobs from Prior `StartTranslateValidateAsync` Calls
...
**What happens:**
1. Old `TranslationValidationJob` instances (for the previous run IDs) are still queued in Hangfire.
2. The cascade reset cleared `ValidationRunIds`, so the old run IDs are no longer in the session.
3. `StartTranslateValidateAsync` creates new `TranslationValidationRun` rows and enqueues new jobs.
4. Old jobs complete → `TryUpdateSessionStatusAsync` loads sessions referencing that old run ID → finds no session referencing the old IDs (they were overwritten) → does nothing.
5. Old runs remain in the DB linked to the draft talk, accessible via the talk's validation history.

**No active cancellation exists anywhere in the codebase for old Hangfire jobs.**

**Risk:** In a high-throughput scenario (e.g., user rapidly re-submits SettingsStep), many runs and jobs pile up for the same draft talk. Each job competes to write `ToolboxTalkTranslation` rows for the same `(TalkId, LanguageCode)` pair. The hard-delete at the start of `StartTranslateValidateAsync` reduces (but doesn't eliminate) the collision window.

### 6.5 Session Stuck in `TranslatingValidating`

**Scenario:** A `TranslationValidationJob` throws an exception that escapes before setting `run.Status = Failed`. Hangfire retries once (retry count = 1). On second failure, Hangfire marks the job as failed in its own storage. The run row status in our DB is whatever the last in-progress state was (possibly still `Running` or `Pending`).

**Effect:** `TryUpdateSessionStatusAsync` loads run statuses and finds at least one that is not `Completed` or `Failed` — `allComplete = false` — so the session stays `TranslatingValidating` indefinitely.

**Recovery path:** None until session expiry (`ExpiresAt`). `ExpiredSessionCleanupJob` will eventually mark it `Abandoned`. There is no manual "force-validate" or "force-fail" endpoint.
...
**Exception:** `GenerateQuizAsync` accepts `GeneratingQuiz` as an input status (line 944), which is also an in-flight state. If quiz generation is interrupted mid-status-write, a subsequent call finding the session in `GeneratingQuiz` will re-run quiz generation. This is the only auto-recovery path in the service.

### 6.7 `TryUpdateSessionStatusAsync` Relies on String-Matching of JSON

The check at [TranslationValidationJob.cs:954](src/Modules/ToolboxTalks/QuantumBuild.Modules.ToolboxTalks.Infrastructure/Jobs/TranslationValidationJob.cs#L954):
```csharp
if (session.ValidationRunIds == null || !session.ValidationRunIds.Contains(runIdString))
    continue;
```

This is a substring check on the raw JSON string before deserializing. It works as long as `Guid.ToString()` produces a format that matches the JSON serialization format. If there is any formatting difference (e.g., uppercase vs lowercase Guid strings in JSON), the substring check passes but `runIds.Contains(validationRunId)` at line 968 fails, and the session never transitions.
...
### 6.10 `UpdateQuestionsAsync` Silently Demotes `Validated` to `QuizGenerated`

**Where:** `UpdateQuestionsAsync` ([service:1087–1088](src/Modules/ToolboxTalks/QuantumBuild.Modules.ToolboxTalks.Infrastructure/Services/ContentCreation/ContentCreationSessionService.cs#L1087)).

**What:** The status-promotion guard `if (session.Status != ContentCreationSessionStatus.QuizGenerated) session.Status = QuizGenerated` fires for ALL non-`QuizGenerated` statuses, including `Validated`. The code comment reads "for the first time (no prior generation)" — but no `QuestionsJson` check is performed. Editing questions from `Validated` demotes the session back to `QuizGenerated` without clearing `ValidationRunIds`.

**Consequence:** The wizard's step indicator shows steps 5 and 6 as reachable (because `ValidationRunIds` is still non-null), but the session is now `QuizGenerated`. The next `StartTranslateValidateAsync` call will re-sync the updated questions and hard-delete stale translations. Until then, existing translations do not reflect the new questions.

**Risk:** If the user navigates directly to Publish (step 7) without re-running translation, updated questions are synced at publish time via `SyncQuizQuestionsToTalk`, but no re-translation occurs. Employees see questions translated in the old configuration.

### 6.11 `ConfirmUploadAsync` Resets to `Draft` From Any Status

**Where:** `ConfirmUploadAsync` ([service:1274](src/Modules/ToolboxTalks/QuantumBuild.Modules.ToolboxTalks.Infrastructure/Services/ContentCreation/ContentCreationSessionService.cs#L1274)).

**What:** The presigned-upload confirmation path has **no status guard**. Line 1274 always sets `Status = Draft`, regardless of current status. If a user uploads a replacement file after reaching `Validated` (or even `Published` status via this path), the session resets to `Draft` while retaining `QuestionsJson`, `ValidationRunIds`, `SettingsJson`, and all downstream DB artefacts.

**Contrast:** `UploadFileAsync` (the direct-upload path) has a `Status == Draft` guard at line 123. `ConfirmUploadAsync` skips this guard entirely.

**Risk:** A re-upload from `Validated` produces a session in `Draft` with a new source file but stale translation runs and stale questions — a state that no normal UI flow creates and that would confuse any code assuming "Draft = no prior work done."

---

## 7. Open Questions

Items marked **[RESOLVED]** were answered by the verification pass on 2026-06-04.
...
**Result:** A user **can** click step 2 (Parse) in the step indicator while the session is `TranslatingValidating`. The ParseStep renders normally. However, if the user tries to **save section edits**, the backend's in-flight guard in `UpdateSectionsAsync` (lines 327–345) throws HTTP 400: "Section editing is not available while TranslatingValidating is in progress." The user can view sections freely; any save attempt is rejected by the backend until the session reaches `Validated`.
...
If the user re-runs `StartTranslateValidateAsync` after cascade reset, lines 668–706 unconditionally call `_subtitleOrchestrator.StartProcessingAsync`, creating a **new** `SubtitleProcessingJob` row and a new Hangfire job. The new subtitle job ID overwrites `session.SubtitleJobId` at line 693. The old subtitle job continues running concurrently, competing to write subtitle files for the same talk with the same language codes. Last writer wins on the R2 object key. No deduplication or cancellation exists.
...
1. **What is the actual `SessionExpiryHours` value?** Set from `_validationSettings.SessionExpiryHours` ([service:82](src/Modules/ToolboxTalks/QuantumBuild.Modules.ToolboxTalks.Infrastructure/Services/ContentCreation/ContentCreationSessionService.cs#L82)) but the default was not confirmed. A short expiry (e.g., 24 hours) combined with long validation runs could cause sessions to expire while in `TranslatingValidating`.
...
- **Data cleared:** `QuestionsJson = null`, `ValidationRunIds = null`, `Status = Parsed`.
...
### 8.6 Edit Quiz Questions from `Validated` (silent status demotion — sharp edge)

- **Data written:** `QuestionsJson` overwritten; `Status` → `QuizGenerated` (see §6.10).
- **Data NOT cleared:** `ValidationRunIds` — old runs still reachable; steps 5 and 6 still show as clickable in the step indicator, giving a false impression that translations are current.
- **DB artefacts:** Draft talk still has old questions; `QuestionsJson` has new questions. Reconciled when `StartTranslateValidateAsync` next runs.
- **Consequence:** If user re-runs translation → new runs created, questions re-synced, stale translations deleted. If user skips to Publish → updated questions synced at publish, but **no re-translation occurs**. Translated question text reflects the old configuration.
...
- **Data NOT cleared:** `QuestionsJson`, `ValidationRunIds`, `SettingsJson`, `SubtitleJobId`, `TargetLanguageCodes`, `TranscriptText`, draft talk.
...
### 8.10 Change Input Source via `ConfirmUploadAsync` from Any Status

- **Endpoint:** `POST /api/toolbox-talks/create/session/{id}/confirm-upload`
- **Data written:** Same as `UpdateSourceAsync` reset plus new `SourceFileUrl`, `SourceFileName`, `SourceFileType`.
- **Consequence:** Identical to §8.9 — see §6.11 for the sharp edge.
...
- **Exception:** `GenerateQuizAsync` accepts `GeneratingQuiz` as input (line 944). A session stuck in `GeneratingQuiz` after an interrupted write can be recovered by re-calling the generate-quiz endpoint. This is the only self-healing re-entry path in the service.
...
| 4 | §2.2 `UpdateSectionsAsync` in-flight guard | Document did not state which statuses are blocked. | Guard covers all five in-flight statuses: `{Transcribing, Parsing, GeneratingQuiz, TranslatingValidating, Publishing}` (lines 327–334). |
...
| 7 | §4 (absent) | Publish path not traced end-to-end. | §4.8 added: full trace of `PublishAsLessonAsync` (both paths) and `PublishAsCourseAsync`. |
...
*Document last revised: 2026-06-04. Verified against commit `21ef65d` (HEAD of `transval` branch). Re-verify file paths and line numbers before relying on them after any significant refactor.*

...
- Unify employee-code generation across creation paths — the employee-creation path always auto-generates the per-tenant EMP### code; the user→employee path requires it entered manually. Make the user→employee path auto-generate by default, with an optional manual override to preserve the legacy-system-code use case. UI retrofit: the EmployeeCode field on the user-create form's employee section becomes optional. (Could be done as one consolidation effort with the throwaway-password unification — both converge divergent user/employee creation paths)
...
- CLAUDE.md accuracy — Employee.EmployeeCode: document that the field is auto-generated (per-tenant, EMP###) on the employee-creation path but currently manually required on the user→employee path. Resolves once the code-generation unification above is done
...
- CLAUDE.md accuracy — two API response conventions: core CRUD controllers return Result<T> envelopes ({success, data, errors}) → frontend reads response.data.data; newer controllers (Monitoring, PipelineAudit, Regulatory, QR, TenantSettings/Sectors/Modules, SafetyGlossary, RequirementMapping) return DTOs directly → frontend reads response.data. Match the controller's existing convention when writing a new API function
